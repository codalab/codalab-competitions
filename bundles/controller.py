...
def reserveNewBundleName():
    ...
    if not os.path.exists(generatedName):
        os.mkdir(generatedName)
    while True:
        name = os.path.join(generatedName, str(i))
        if not os.path.exists(os.path.join(bundlesPath, name)):
            break
        i += 1
    os.mkdir(name)
    # print "Created " + name
...
def createBundle(metadata):
    ...
    if spec in bundleCache:
        return bundleCache[spec]
...
def getHtmlOutFile(stack, name):
    return '-'.join(stack + [name.replace('/', '-')]) + '.html'


def getHtmlOutPath(stack, name):
    return os.path.join(htmlOutPath, getHtmlOutFile(stack, name))
...
def runProgram(bundle):
    ...
    # input is unused currently, but this is how it is retrieved.
...
class Worksheet:
    ...
    def __init__(self, name):
        ...
        if not self.description:
            raise BadFormatException('Missing description')

        self.blocks = []
        self.varToBlock = {}  # Mapping from variable to blocks
        blocks = contents.get('blocks')
        if not blocks:
            raise BadFormatException('Missing blocks')
                ...
                if block.var:
                    if block.var in self.varToBlock:
                        raise BadFormatException(
                            'Variable defined twice: ' + block.var)
    ...
    def resolveReferences(self, key, overrideVarToName, value):
        ...
        if m:
            ...
            if not name:
                ...
                if not refBlock:
                    raise BadFormatException(
                        'Undefined variable reference: ' + refVar)
    ...
    def execute(self, stack=[], overrideVarToName={}):
        ...
        for block in self.blocks:
            if not isinstance(block, BundleBlock):
                continue
            ...
            if block.name:  # Load existing bundle
                ...
                for key, value in block.contents.items():
                    value = self.resolveReferences(
                        key, overrideVarToName, value)
                    info[key] = value
                ...
                # Execute stuff
                if block.program:
                    ok = runProgram(block.bundle)
                if block.macro:
                    ok = runMacro(stack + [block.var], block.bundle)
                if not ok:
                    break
    ...
    def generateHtml(self, stack, overrideVarToName):
        if not os.path.exists(htmlOutPath):
            os.mkdir(htmlOutPath)
        ...
        for block in self.blocks:
            if isinstance(block, TextBlock):
                print >>out, '<p><i><font color="brown">' + \
                    block.contents + '</font></i></p>'
            elif isinstance(block, BundleBlock):
                if block.var in overrideVarToName:  # Substituted bundle
                    self.printBundleHtml(stack, overrideVarToName, block.var, {}, getBundle(
                        overrideVarToName[block.var]), out)
                else:
                    self.printBundleHtml(
                        stack, overrideVarToName, block.var, block.contents, block.bundle, out)
            else:
    ...
    def printBundleHtml(self, stack, overrideVarToName, var, contents, bundle, out):
        ...
        if var in overrideVarToName:
            print >>out, '<p>[<b><font color="blue">' + \
                var + '</font></b>]<br/>'
        else:
            print >>out, '<p>[<b><font color="green">' + \
                var + '</font></b>]<br/>'
        print >>out, '<table>'
        hitKeys = set()
        ...
        for key, value in bundle.metadata.items():
            if key in producedRunKeys:
                continue
            value = str(value)
            generalValue = contents.get(key)
            if generalValue:
                generalValue = str(generalValue)
            ...
            if generalValue and generalValue != value:
                renderedValue = value + \
                    ' [<font color="green">' + generalValue[1:] + '</font>]'
            else:
                renderedValue = value
            if key == 'macro':
                renderedValue = '<a href="' + \
                    getHtmlOutFile(stack + [var], value) + \
                    '">' + renderedValue + '</a>'
