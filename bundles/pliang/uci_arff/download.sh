...
for x in */data.arff; do echo $x; m=`dirname $x`/metadata; d=`grep Title $x | head -1 | cut -f 2 -d :`; echo "title: $d" > $m; echo "description: $d" >> $m; echo "source: http://repository.seasr.org/Datasets/UCI/arff" >> $m; done || exit 1
