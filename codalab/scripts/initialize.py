...
for name, codename, classname in cvs:
    ncv, created = ContentVisibility.objects.get_or_create(
...
for category in ccs:
    nc, created = ContentCategory.objects.get_or_create(
        parent=category['parent'], name=category['name'],
        codename=category[
            'codename'], visibility=category['visibility'],
...
for dci in cis:
    dcii, created = DefaultContentItem.objects.get_or_create(
        category=dci['category'], label=dci['label'],
        rank=dci['rank'], required=dci[
            'required'], codename=dci['codename'],
...
for name, codename, description in pss:
    pstatus, created = ParticipantStatus.objects.get_or_create(
...
for name, codename in submission_status_set:
    pstatus, created = CompetitionSubmissionStatus.objects.get_or_create(
