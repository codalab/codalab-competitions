...
def analytics_detail(request):
    ...
    for date in User.objects.all().dates('date_joined', 'month'):
        ...
        users_joined_this_month = User.objects.filter(
            date_joined__month=date.month,
            date_joined__year=date.year).aggregate(
            Count('pk'))
        monthly_total_users_joined[
            date.year][
            date.strftime("%B")] = users_joined_this_month['pk__count']
    ...
    return render(
        ...
        "analytics/analytics.html",
        {
            ...
            'competitions_published_count': Competition.objects.filter(
