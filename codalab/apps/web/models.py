...
class PageContainer(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.name = "%s - %s" % (self.owner.__unicode__(),
                                 self.name if self.name else str(
            self.pk))
...
class Competition(models.Model):
    ...
    creator = models.ForeignKey(
        ...
        related_name='competitioninfo_creator')
    admins = models.ManyToManyField(
        settings.AUTH_USER_MODEL,
        related_name='competition_admins',
        blank=True,
        null=True)
    modified_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name='competitioninfo_modified_by')
    ...
    allow_public_submissions = models.BooleanField(
        default=False,
        verbose_name="Allow sharing of public submissions")
    ...
    def get_start_date(self):
        ...
        if len(phases) > 0:
            ...
            return datetime.datetime.strptime(
                '26 Sep 2012', '%d %b %Y').replace(tzinfo=None)

    @property
    def is_active(self):
        if self.end_date is None:
            return True
        if isinstance(self.end_date, datetime.datetime.date):
    ...
    def do_phase_migration(self, current_phase, last_phase):
        ...
        '''
        logger.info(
            "Checking for submissions that may still be running competition pk=%s" %
            self.pk)

        if last_phase.submissions.filter(
                status__codename=CompetitionSubmissionStatus.RUNNING).exists():
            logger.info(
                'Some submissions still marked as processing for competition pk=%s' %
                self.pk)
            ...
            logger.info(
                "No submissions running for competition pk=%s" %
                self.pk)

        logger.info(
            'Doing phase migration on competition pk=%s from phase: %s to phase: %s' %
            (self.pk, last_phase.phasenumber, current_phase.phasenumber))

        if self.is_migrating:
            logger.info(
                'Trying to migrate competition pk=%s, but it is already being migrated!' %
                self.pk)
            return
        ...
        try:
            ...
            leader_board_entries = PhaseLeaderBoardEntry.objects.filter(
                board=leader_board)
        ...
        # this should really be True until evaluate_submission tasks are all
        # the way completed
    ...
    def check_trailing_phase_submissions(self):
        ...
        if self.is_migrating:
            logger.info(
                'Trying to check migrations on competition pk=%s, but it is already being migrated!' %
                self.pk)
            return
        ...
        logger.info(
            "Checking for needed migrations on competition pk=%s, last phase: %s, current phase: %s" %
            (self.pk, last_phase.phasenumber, current_phase.phasenumber))
    ...
    def get_results_csv(self, phase_pk):
        ...
        for group in groups:
            ...
            headers = ["User"]
            sub_headers = [""]
            # This ordering dict will contain {<header key>: <order of the
            # column>}
            ordering = {}
            ...
            if len(group['scores']) <= 0:
                ...
                for pk, scores in group['scores']:
                    ...
                    for v in scores['values']:
                        if 'rnk' in v:
                            # Based on the header label insert the score into
                            # the proper column
                            row[ordering[v['name']] +
                                1] = "%s (%s)" % (v['val'], v['rnk'])
                        else:
                            row[ordering[v['name']] +
                                1] = "%s (%s)" % (v['val'], v['hidden_rnk'])
...
def phase_prefix(phase):
    return os.path.join(
        competition_prefix(phase.competition), str(phase.phasenumber))


def phase_data_prefix(phase):
    return os.path.join(
        "competition", str(
            phase.competition.id), str(
            phase.phasenumber), "data")
...
def submission_detailed_results_filename(
        instance, filename="detailed_results.html"):
...
def submission_private_output_filename(
        instance, filename="private_output.zip"):
    return os.path.join(submission_root(instance), "run", filename)
...
def predict_submission_stdout_filename(
        instance, filename="prediction_stdout_file.txt"):
    return os.path.join(submission_root(instance), "pred", "run", filename)


def predict_submission_stderr_filename(
        instance, filename="prediction_stderr_file.txt"):
    return os.path.join(submission_root(instance), "pred", "run", filename)
...
class CompetitionPhase(models.Model):
    ...
    max_submissions = models.PositiveIntegerField(
        default=100,
        verbose_name="Maximum Submissions (per User)")
    max_submissions_per_day = models.PositiveIntegerField(
        default=999,
        verbose_name="Max Submissions (per User) per day")
    is_scoring_only = models.BooleanField(
        default=True,
        verbose_name="Results Scoring Only")
    scoring_program = models.FileField(
        upload_to=phase_scoring_program_file,
        storage=BundleStorage,
        null=True,
        blank=True,
        verbose_name="Scoring Program")
    reference_data = models.FileField(
        upload_to=phase_reference_data_file,
        storage=BundleStorage,
        null=True,
        blank=True,
        verbose_name="Reference Data")
    input_data = models.FileField(
        upload_to=phase_input_data_file,
        storage=BundleStorage,
        null=True,
        blank=True,
    ...
    datasets = models.ManyToManyField(
        ...
        blank=True,
        related_name='phase')
    leaderboard_management_mode = models.CharField(
        max_length=50,
        default=LeaderboardManagementMode.DEFAULT,
        verbose_name="Leaderboard Mode")
    ...
    execution_time_limit = models.PositiveIntegerField(
        default=(
            5 *
            60),
        verbose_name="Execution time limit (in seconds)")
    ...
    input_data_organizer_dataset = models.ForeignKey(
        'OrganizerDataSet',
        null=True,
        blank=True,
        related_name="input_data_organizer_dataset",
        verbose_name="Input Data",
        on_delete=models.SET_NULL)
    reference_data_organizer_dataset = models.ForeignKey(
        'OrganizerDataSet',
        null=True,
        blank=True,
        related_name="reference_data_organizer_dataset",
        verbose_name="Reference Data",
        on_delete=models.SET_NULL)
    scoring_program_organizer_dataset = models.ForeignKey(
        'OrganizerDataSet',
        null=True,
        blank=True,
        related_name="scoring_program_organizer_dataset",
        verbose_name="Scoring Program",
        on_delete=models.SET_NULL)
    ...
    def scores(self, include_scores_not_on_leaderboard=False, **kwargs):
        ...
        if not created:
            if include_scores_not_on_leaderboard:
                ...
                for submission in qs:
                    ...
                    submissions.append(
                        (submission.pk, submission.participant.user))
            else:
                ...
                for entry in qs:
                    submissions.append(
                        (entry.result.id, entry.result.participant.user))

        results = []
        for count, g in enumerate(
            SubmissionResultGroup.objects.filter(
                phases__in=[self]).order_by('ordering')):
            label = g.label
            headers = []
            scores = {}
            ...
            for x in SubmissionScoreSet.objects.order_by(
                ...
                if x.parent is not None:
                    ...
                    columnSubLabels = [
                        {'key': x.key, 'label': x.label, 'ordering': x.ordering}]
                else:
                    columnKey = x.key
                    columnLabel = x.label
                ...
                if columnKey not in columnKeys:
                    ...
                    headers.append({'key': columnKey,
                                    'label': columnLabel,
                                    'subs': columnSubLabels,
                                    'ordering': columnOrdering})
                else:
                    headers[
                        columnKeys[columnKey]]['subs'].extend(columnSubLabels)
            ...
            for i in range(len(scoreDefs)):
                if (selection_key is None) or (
                        scoreDefs[i].selection_default > selection_order):
        ...
        if len(submissions) > 0:
            ...
            # not_computed_scoredefs: map (scoredef.id, scoredef) to keep track
            # of non-computed scoredefs
            ...
            for result in results:
                ...
                if len(computed_scoredef_ids) > 0:
                    ...
                    fields = SubmissionComputedScoreField.objects.filter(
                        computed_id__in=computed_ids).select_related(
                        'scoredef',
                        'computed')
                    for field in fields:
                        ...
                        computed_deps[
                            field.computed.scoredef_id].append(
                            field.scoredef)
            ...
            scoredef_ids = [
                sdef_id for (
                    sdef_id,
                    sdef) in not_computed_scoredefs.iteritems()]
            for s in SubmissionScore.objects.filter(
                    scoredef_id__in=scoredef_ids,
            ...
            for result in results:
                ...
                ranked_submissions = sorted(
                    submission_ids,
                    cmp=CompetitionPhase.rank_submissions(overall_ranks))
...
class CompetitionSubmission(models.Model):
    ...
    file = models.FileField(
        upload_to=submission_file_name,
        storage=BundleStorage,
        null=True,
        blank=True)
    ...
    inputfile = models.FileField(
        upload_to=submission_inputfile_name,
        storage=BundleStorage,
        null=True,
        blank=True)
    runfile = models.FileField(
        upload_to=submission_runfile_name,
        storage=BundleStorage,
        null=True,
        blank=True)
    ...
    output_file = models.FileField(
        upload_to=submission_output_filename,
        storage=BundleStorage,
        null=True,
        blank=True)
    private_output_file = models.FileField(
        upload_to=submission_private_output_filename,
        storage=BundleStorage,
        null=True,
        blank=True)
    stdout_file = models.FileField(
        upload_to=submission_stdout_filename,
        storage=BundleStorage,
        null=True,
        blank=True)
    stderr_file = models.FileField(
        upload_to=submission_stderr_filename,
        storage=BundleStorage,
        null=True,
        blank=True)
    history_file = models.FileField(
        upload_to=submission_history_file_name,
        storage=BundleStorage,
        null=True,
        blank=True)
    scores_file = models.FileField(
        upload_to=submission_scores_file_name,
        storage=BundleStorage,
        null=True,
        blank=True)
    detailed_results_file = models.FileField(
        upload_to=submission_detailed_results_filename,
        storage=BundleStorage,
        null=True,
        blank=True)
    prediction_runfile = models.FileField(
        upload_to=submission_prediction_runfile_name,
        storage=BundleStorage,
        null=True,
        blank=True)
    prediction_output_file = models.FileField(
        upload_to=submission_prediction_output_filename,
        storage=BundleStorage,
        null=True,
        blank=True)
    exception_details = models.TextField(blank=True, null=True)
    prediction_stdout_file = models.FileField(
        upload_to=predict_submission_stdout_filename,
        storage=BundleStorage,
        null=True,
        blank=True)
    prediction_stderr_file = models.FileField(
        upload_to=predict_submission_stderr_filename,
        storage=BundleStorage,
        null=True,
        blank=True)
    ...
    organization_or_affiliation = models.CharField(
        max_length=255,
        null=True,
        blank=True)
    ...
    def save(self, ignore_submission_limits=False, *args, **kwargs):
        ...
        if not self.pk:
            if not ignore_submission_limits:
                ...
                subnum = CompetitionSubmission.objects.filter(
                    phase=self.phase,
                    participant=self.participant).aggregate(
                ...
                failed_count = CompetitionSubmission.objects.filter(
                    phase=self.phase,
                    participant=self.participant,
                    status__name=CompetitionSubmissionStatus.FAILED).count()

                print "This is submission number %d, and %d submissions have failed" % (self.submission_number,
                                                                                        failed_count)
                offset_submission_count = self.submission_number - failed_count

                if (offset_submission_count > self.phase.max_submissions):
                    print "Checking to see if the offset_submission_count (%d) is greater than the maximum " \
                          "allowed (%d)" % (offset_submission_count, self.phase.max_submissions)
                ...
                if hasattr(self.phase, 'max_submissions_per_day'):
                    ...
                    submissions_from_today_count = len(
                        CompetitionSubmission.objects.filter(
                            phase__competition=self.phase.competition,
                            participant=self.participant,
                            submitted_at__gte=datetime.date.today()))

                    print 'Count is %s and maximum is %s' % (submissions_from_today_count,
                                                             self.phase.max_submissions_per_day)

                    if submissions_from_today_count + 1 - \
                            failed_count > self.phase.max_submissions_per_day or \
                                    self.phase.max_submissions_per_day == 0:
                        print 'PERMISSION DENIED'
                        raise PermissionDenied(
                            "The maximum number of submissions this day have been reached.")

            self.status = CompetitionSubmissionStatus.objects.get_or_create(
                codename=CompetitionSubmissionStatus.SUBMITTING)[0]
    ...
    def get_filename(self):
        ...
        name = ''
        try:
            name = self.file.storage.properties(
                self.file.name)['x-ms-meta-name']
        except:
            pass
        if len(name) == 0:
            # For backwards compat, fallback to this method of getting the
            # name.
    ...
    def get_file_for_download(self, key, requested_by):
        ...
        file_attr, file_ext, file_has_restricted_access = downloadable_files[
            key]
...
class SubmissionResultGroup(models.Model):
    ...
    phases = models.ManyToManyField(
        CompetitionPhase,
        through='SubmissionResultGroupPhase')
...
class SubmissionScoreDef(models.Model):
    ...
    label = models.CharField(max_length=50)
    sorting = models.SlugField(
        max_length=20, default='asc', choices=(
    ...
    groups = models.ManyToManyField(
        ...
        through='SubmissionScoreDefGroup')
    ordering = models.PositiveIntegerField(default=1)
...
class CompetitionDefBundle(models.Model):
    ...
    def unpack(self):
        ...
        logger.debug(
            "CompetitionDefBundle::unpack creating base competition (pk=%s)",
            self.pk)
        comp_spec_file = [x for x in zf.namelist() if ".yaml" in x][0]
        yaml_contents = zf.open(comp_spec_file).read()

        # Forcing YAML to interpret the file while maintaining the original
        # order things are in
        ...
        if 'end_date' in comp_base:
            if comp_base['end_date'] is None:
                ...
                comp_base['end_date'] = CompetitionDefBundle.localize_datetime(
                    comp_base['end_date'])
        ...
        comp.save()
        logger.debug(
            "CompetitionDefBundle::unpack created base competition (pk=%s)",
            self.pk)

        if admin_names:
            logger.debug(
                "CompetitionDefBundle::unpack looking up admins %s",
                comp_spec['admin_names'])
            admins = User.objects.filter(username__in=admin_names.split(','))
            logger.debug(
                "CompetitionDefBundle::unpack found admins %s",
                admins)
            comp.admins.add(*admins)

            logger.debug(
                "CompetitionDefBundle::unpack adding admins as participants")
            approved_status = ParticipantStatus.objects.get(
        ...
        if 'image' in comp_base:
            comp.image.save(
                comp_base['image'], File(
                    io.BytesIO(
                        zf.read(
                            comp_base['image']))))
            comp.save()
            logger.debug(
                "CompetitionDefBundle::unpack saved competition logo (pk=%s)",
                self.pk)
        ...
        Page.objects.create(
            ...
            rank=2,
            html=zf.read(
                comp_spec['html']['terms']))

        default_pages = ('overview', 'evaluation', 'terms', 'data')

        for (page_number, (page_name, page_data)) in enumerate(
                comp_spec['html'].items()):
            if page_name not in default_pages:
                Page.objects.create(
                    ...
                    label=page_name,
                    rank=3 + page_number,
                    # Start at 3 (Overview, Evaluation and Terms and Conditions
                    # first)
        ...
        Page.objects.create(
            ...
            html="")
        logger.debug(
            "CompetitionDefBundle::unpack created competition pages (pk=%s)",
            self.pk)
        ...
        for index, p_num in enumerate(comp_spec['phases']):
            ...
            if 'leaderboard_management_mode' in phase_spec:
                if not LeaderboardManagementMode.is_valid(
                        phase_spec['leaderboard_management_mode']):
                    ...
                    logger.warn(
                        msg.format(
                            ...
                            p_num))
                    phase_spec[
                        'leaderboard_management_mode'] = LeaderboardManagementMode.DEFAULT
            else:
                phase_spec[
                    'leaderboard_management_mode'] = LeaderboardManagementMode.DEFAULT
            ...
            phase_spec['start_date'] = CompetitionDefBundle.localize_datetime(
                phase_spec['start_date'])
            ...
            if index == 0:
                ...
                logger.debug(
                    'Set last_phase_migration to #%s' %
                    phase_spec['phasenumber'])
                comp.save()

            phase, created = CompetitionPhase.objects.get_or_create(
                **phase_spec)
            logger.debug(
                "CompetitionDefBundle::unpack created phase (pk=%s)",
                self.pk)
            ...
            if hasattr(phase, 'scoring_program') and phase.scoring_program:
                if phase_spec["scoring_program"].endswith(".zip"):
                    phase.scoring_program.save(
                        phase_scoring_program_file(phase),
                        File(
                            io.BytesIO(
                                zf.read(
                                    phase_spec['scoring_program']))))

                    file_name = os.path.splitext(
                        os.path.basename(
                            phase_spec['scoring_program']))[0]
                    if phase_spec['scoring_program'] not in data_set_cache:
                        logger.debug(
                            'Adding organizer dataset to cache: %s' %
                            phase_spec['scoring_program'])
                        data_set_cache[
                            phase_spec['scoring_program']] = OrganizerDataSet.objects.create(
                            name="%s_%s_%s" %
                            (file_name,
                             phase.phasenumber,
                             comp.pk),
                    ...
                    phase.scoring_program_organizer_dataset = data_set_cache[
                        phase_spec['scoring_program']]
                else:
                    logger.debug(
                        "CompetitionDefBundle::unpack getting dataset for scoring_program with key %s",
                        phase_spec["scoring_program"])
                    data_set = OrganizerDataSet.objects.get(
                        key=phase_spec["scoring_program"])
            ...
            if hasattr(phase, 'reference_data') and phase.reference_data:
                if phase_spec["reference_data"].endswith(".zip"):
                    phase.reference_data.save(
                        phase_reference_data_file(phase),
                        File(
                            io.BytesIO(
                                zf.read(
                                    phase_spec['reference_data']))))

                    file_name = os.path.splitext(
                        os.path.basename(
                            phase_spec['reference_data']))[0]
                    if phase_spec['reference_data'] not in data_set_cache:
                        logger.debug(
                            'Adding organizer dataset to cache: %s' %
                            phase_spec['reference_data'])
                        data_set_cache[
                            phase_spec['reference_data']] = OrganizerDataSet.objects.create(
                            name="%s_%s_%s" %
                            (file_name,
                             phase.phasenumber,
                             comp.pk),
                            ...
                            uploaded_by=self.owner)
                    phase.reference_data_organizer_dataset = data_set_cache[
                        phase_spec['reference_data']]
                else:
                    logger.debug(
                        "CompetitionDefBundle::unpack getting dataset for reference_data with key %s",
                        phase_spec["reference_data"])
                    data_set = OrganizerDataSet.objects.get(
                        key=phase_spec["reference_data"])
            ...
            if 'input_data' in phase_spec:
                if phase_spec["input_data"].endswith(".zip"):
                    phase.input_data.save(
                        phase_input_data_file(phase), File(
                            io.BytesIO(
                                zf.read(
                                    phase_spec['input_data']))))

                    file_name = os.path.splitext(
                        os.path.basename(
                            phase_spec['input_data']))[0]
                    if phase_spec['input_data'] not in data_set_cache:
                        logger.debug(
                            'Adding organizer dataset to cache: %s' %
                            phase_spec['input_data'])
                        data_set_cache[
                            phase_spec['input_data']] = OrganizerDataSet.objects.create(
                            name="%s_%s_%s" %
                            (file_name,
                             phase.phasenumber,
                             comp.pk),
                            ...
                            uploaded_by=self.owner)
                    phase.input_data_organizer_dataset = data_set_cache[
                        phase_spec['input_data']]
                else:
                    logger.debug(
                        "CompetitionDefBundle::unpack getting dataset for input_data with key %s",
                        phase_spec["input_data"])
                    data_set = OrganizerDataSet.objects.get(
                        key=phase_spec["input_data"])
                    phase.input_data = data_set.data_file.file.name
                    phase.input_data_organizer_dataset = data_set

            phase.auto_migration = bool(
                phase_spec.get(
                    'auto_migration',
                    False))
            phase.save()
            logger.debug(
                "CompetitionDefBundle::unpack saved scoring program and reference data (pk=%s)",
                self.pk)
            ...
            for ds in datasets.keys():
                ...
                count += 1
            logger.debug(
                "CompetitionDefBundle::unpack saved datasets (pk=%s)",
                self.pk)

        logger.debug(
            "CompetitionDefBundle::unpack saved created competition phases (pk=%s)",
            self.pk)
        ...
        if 'leaderboard' in comp_spec:
            ...
            if 'leaderboards' in comp_spec['leaderboard']:
                ...
                for key, value in comp_spec[
                        'leaderboard']['leaderboards'].items():
                    rg, cr = SubmissionResultGroup.objects.get_or_create(competition=comp,
                    ...
                    for gp in comp.phases.all():
                        rgp, crx = SubmissionResultGroupPhase.objects.get_or_create(
                            phase=gp, group=rg)
            logger.debug(
                "CompetitionDefBundle::unpack created leaderboard (pk=%s)",
                self.pk)

            # Create score groups
            if 'column_groups' in comp_spec['leaderboard']:
                groups = {}
                for key, vals in comp_spec['leaderboard'][
                        'column_groups'].items():
                    index = comp_spec['leaderboard'][
                        'column_groups'].keys().index(key) + 1
                    if vals is None:
                        vals = dict()
                    setdefaults = {
                    ...
                    s, cr = SubmissionScoreSet.objects.get_or_create(
                        competition=comp, key=key.strip(), defaults=setdefaults)
                    groups[s.label] = s
            logger.debug(
                "CompetitionDefBundle::unpack created score groups (pk=%s)",
                self.pk)
            ...
            if 'columns' in comp_spec['leaderboard']:
                ...
                for key, vals in comp_spec['leaderboard']['columns'].items():
                    ...
                    if 'column_group' in vals:
                        ...
                        g, cr = SubmissionScoreSet.objects.get_or_create(
                            competition=comp, parent=gparent, key=sd.key, defaults=dict(
                                scoredef=sd, label=sd.label, ordering=sd.ordering))
                    else:
                        g, cr = SubmissionScoreSet.objects.get_or_create(
                            competition=comp, key=sd.key, defaults=dict(
                                scoredef=sd, label=sd.label, ordering=sd.ordering))
                ...
                for key, vals in comp_spec['leaderboard']['columns'].items():
                    ...
                    for f in vals['computed']['fields'].split(","):
                        ...
                        # This is not a safe assumption -- given we can't
                        # control key/value ordering in a dictionary.
                    ...
                    SubmissionScoreDefGroup.objects.create(
                        ...
                        group=leaderboards[
                            vals['leaderboard']['label']])
                logger.debug(
                    "CompetitionDefBundle::unpack created scores (pk=%s)",
                    self.pk)
        ...
        resulting_participant, created = CompetitionParticipant.objects.get_or_create(
            user=self.owner, competition=comp, defaults={
                'status': approved})
        logger.debug(
            "CompetitionDefBundle::unpack added owner as participant (pk=%s)",
            self.pk)
...
class SubmissionScoreSet(MPTTModel):
    ...
    def __unicode__(self):
        return "%s %s" % (
            self.parent.label if self.parent else None, self.label)
...
class PhaseLeaderBoard(models.Model):
    ...
    def submissions(self):
        return CompetitionSubmission.objects.filter(
            leaderboard_entry_result__board=self)

    def __unicode__(self):
        return "%s [%s]" % (
            self.phase.label, 'Open' if self.is_open else 'Closed')
    ...
    def scores(self, **kwargs):
        return self.phase.scores(
            score_filters=dict(result__leaderboard_entry_result__board=self))
...
class PhaseLeaderBoardEntry(models.Model):
    ...
    result = models.ForeignKey(
        ...
        related_name='leaderboard_entry_result')
...
def dataset_data_file(dataset, filename="data.zip"):
    return os.path.join(
        "datasets", str(dataset.pk), str(uuid.uuid4()), filename)
...
class OrganizerDataSet(models.Model):
    ...
    sub_data_files = models.ManyToManyField(
        'OrganizerDataSet',
        null=True,
        blank=True,
        verbose_name="Bundle of data files")
...
def add_submission_to_leaderboard(submission):
    ...
    for entry in entries:
        entry.delete()
    lbe, created = PhaseLeaderBoardEntry.objects.get_or_create(
        board=lb, result=submission)
    return lbe, created
