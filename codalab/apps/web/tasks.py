...
def create_competition_task(job_id, args):
    ...
    def create_it(job):
        ...
        logger.info(
            "Creating competition for competition bundle (bundle_id=%s, job_id=%s)",
            comp_def_id,
            job.id)
        ...
        return JobTaskResult(
            status=Job.FINISHED, info={'competition_id': competition.pk})
...
def create_competition(comp_def_id):
    ...
    Returns a Job object which can be used to track the progress of the operation.
    """
    return Job.objects.create_and_dispatch_job(
        'create_competition', {'comp_def_id': comp_def_id})
...
def _set_submission_status(submission_id, status_codename):
    ...
    with transaction.commit_on_success():
        ...
        if old_status_codename not in _FINAL_STATES:
            ...
            logger.info(
                "Skipping update of submission status: invalid transition %s -> %s  (id=%s).",
                status_codename,
                old_status_codename,
                submission_id)
...
def score(submission, job_id):
    ...
    submission.scores_file.save(
        'scores.txt', ContentFile(
            submission.phase.competition.get_results_csv(
                submission.phase.pk)))
    ...
    lines.append(
        "submitted-at: %s" %
        submission.submitted_at.replace(
    ...
    # Create stdout.txt & stderr.txt
    if not has_generated_predictions:
        username = submission.participant.user.username
        lines = [
            "Standard output for submission #{0} by {1}.".format(
                submission.submission_number,
...
def update_submission_task(job_id, args):
    ...
    def update_submission(submission, status, job_id, traceback=None):
        ...
        if status == 'finished':
            ...
            if len(submission.execution_key) > 0:
                logger.debug(
                    "update_submission_task loading state: %s",
            ...
            if 'score' in state:
                ...
                logger.debug(
                    "Processing scores... (submission_id=%s)",
                    submission.id)
                for line in scores.split("\n"):
                    if len(line) > 0:
                        label, value = line.split(":")
                        logger.debug(
                            "Attempting to submit score %s:%s" %
                            (label, value))
                        try:
                            ...
                            logger.warning(
                                ...
                                submission.id)
                logger.debug(
                    "Done processing scores... (submission_id=%s)",
                    submission.id)
                _set_submission_status(
                    submission.id,
                    CompetitionSubmissionStatus.FINISHED)
                ...
                if submission.phase.is_blind:
                    logger.debug(
                        "Adding to leaderboard... (submission_id=%s)",
                        submission.id)
        ...
        if status != 'failed':
            logger.error(
                "Invalid status: %s (submission_id=%s)",
                status,
                submission.id)
    ...
    def update_it(job):
        ...
        logger.debug(
            "Entering update_submission_task::update_it (job_id=%s)",
            job.id)
        if job.task_type != 'evaluate_submission':
            raise ValueError(
                "Job has incorrect task_type (job.task_type=%s)",
                job.task_type)
        task_args = job.get_task_args()
        submission_id = task_args['submission_id']
        logger.debug(
            "Looking for submission (job_id=%s, submission_id=%s)",
            job.id,
            submission_id)
...
def evaluate_submission_task(job_id, args):
    ...
    def submit_it():
        ...
        logger.debug(
            "evaluate_submission_task submission_id=%s (job_id=%s)",
            submission_id,
            job_id)
        predict_and_score = args['predict']
        logger.debug(
            "evaluate_submission_task predict_and_score=%s (job_id=%s)",
            predict_and_score,
            job_id)
        submission = CompetitionSubmission.objects.get(pk=submission_id)

        task_name, task_func = (
            'prediction', predict) if predict_and_score else (
            'scoring', score)
        try:
            logger.debug(
                "evaluate_submission_task dispatching %s task (submission_id=%s, job_id=%s)",
                task_name,
                submission_id,
                job_id)
            task_func(submission, job_id)
            logger.debug(
                "evaluate_submission_task dispatched %s task (submission_id=%s, job_id=%s)",
                task_name,
                submission_id,
                job_id)
        except Exception:
            logger.exception(
                "evaluate_submission_task dispatch failed (job_id=%s, submission_id=%s)",
                job_id,
                submission_id)
...
def evaluate_submission(submission_id, is_scoring_only):
    ...
    task_args = {
        ...
        'predict': (
            not is_scoring_only)}
    return Job.objects.create_and_dispatch_job(
        'evaluate_submission', task_args)


def _send_mass_html_mail(
        datatuple,
        fail_silently=False,
        user=None,
        password=None,
...
def send_mass_email_task(job_id, task_args):
    ...
    text = render_to_string(
        "emails/notifications/participation_organizer_direct_email.txt",
        context)
    html = render_to_string(
        "emails/notifications/participation_organizer_direct_email.html",
        context)
...
def send_mass_email(
        competition, body=None, subject=None, from_email=None, to_emails=None):
    task_args = {
