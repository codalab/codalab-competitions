...
a+"/submission",type:"post",cache:!1,data:{id:e,name:b.name,type:b.type,size:b.size,submission_name:$("#submissionName").val(),submission_description:$("#submissionDescription").val()}}).done(function(a){$("#details").html("");$("#user_results tr.noData").remove();$("#user_results").append(d.displayNewSubmission(a));$("#user_results #"+a.id+" .fi-plus").on("click",function(){d.showOrHideSubmissionDetails(this)});$("#fileUploadButton").removeClass("disabled");$("#user_results #"+a.id+" .fi-plus").click()}).fail(function(a){var b=
"An unexpected error occured.";403==a.status&&(b=a.responseJSON.detail);$("#details").html(b);$("#fileUploadButton").removeClass("disabled")})}});$("#user_results .fi-plus").on("click",function(){d.showOrHideSubmissionDetails(this)})},error:function(a,b,e){$(".competition_submissions").html("<div class='alert-error'>An error occurred. Please try refreshing the page.</div>")}})};d.getPhaseResults=function(a,b){$(".competition_results").html("").append("<div class='competitionPreloader'></div>").children().css({top:"200px",
display:"block"});$.ajax({type:"GET",url:"/competitions/"+a+"/results/"+b,cache:!1,success:function(a){$(".competition_results").html("").append(a);$(".column-selectable").click(function(a){a=$(this).closest("table");$(a).find(".column-selected").removeClass();$(this).addClass("column-selected");var b=$(this).attr("name");a=a.find("td").filter(function(){return $(this).attr("name")===b}).addClass("column-selected");if(0<a.length)for(var e=a.slice().sort(function(a,b){var e=parseInt($.text([$(a).find("span")]));
isNaN(e)&&(e=1E5);var c=parseInt($.text([$(b).find("span")]));isNaN(c)&&(c=1E5);return e-c}),c=a[0].parentNode.parentNode,e=e.map(function(){return this.parentNode.cloneNode(!0)}),d=0;d<e.length;d++)$(e[d]).find("td.row-position").text($(e[d]).find("td.column-selected span").text()),c.insertBefore(e[d],a[d].parentNode),c.removeChild(a[d].parentNode)})},error:function(a,b,c){$(".competition_results").html("<div class='alert-error'>An error occurred. Please try refreshing the page.</div>")}})};d.registationCanProceed=
function(){!0===$("#checkbox").is(":checked")?$("#participateButton").removeClass("disabledStatus"):$("#participateButton").addClass("disabledStatus")};d.displayRegistrationStatus=function(a){var b="";$.ajax({type:"GET",url:"/api/competition/"+a+"/mystatus/",cache:!1,success:function(a){console.log(a);"approved"==a.status?location.reload():"pending"==a.status?(b="<div class='participateInfoBlock pendingApproval'>",b+="<div class='infoStatusBar'></div>",b+="<div class='labelArea'>",b+="<label class='regApprovLabel'>Your request to participate in this challenge has been received and a decision is pending.</label>",
...
f).toString();var h=f.getHours().toString(),l=c(f.getMinutes()),c=c(f.getSeconds());$(this).html(b+" "+h+":"+l+":"+c);break;case 4:$(this).html(a.name);break;case 5:$(this).html(d.getSubmissionStatus(a.status));break;case 6:$(this).html("")}});return b};d.oddOrEven=function(a){return a&1?"odd":"even"};d.getSubmissionStatus=function(a){var b="Unknown";"submitting"===a?b="Submitting":"submitted"===a?b="Submitted":"running"===a?b="Running":"failed"===a?b="Failed":"cancelled"===a?b="Cancelled":"finished"===
a&&(b="Finished");return b};d.showOrHideSubmissionDetails=function(a){var b=$(a).parents("tr")[0];if($(a).hasClass("fi-minus"))$(a).removeClass("fi-minus"),$(a).addClass("fi-plus"),$(b).next("tr.trDetails").remove();else{$(a).removeClass("fi-plus");$(a).addClass("fi-minus");a=$("#submission_details_template .trDetails").clone();a.find(".tdDetails").attr("colspan",b.cells.length);a.find("a").each(function(){$(this).attr("href",$(this).attr("href").replace("_",b.id))});a.find("p").each(function(){this.innerText=
this.innerText.replace("__description",$(b).data("description")?$(b).data("description"):"No description given.")});var e=$("#phasestate").val(),k=$(b).find("input[name='state']").val();if(1==e&&1==k){var g=a.find("button");g.removeClass("hide");var e=$(b).find(".status").hasClass("submitted"),h=$("#competitionId").val();f(g,e);g.on("click",function(){c(h,b.id,$("#cstoken").val(),g)})}else{e=$.trim($(b).find(".statusName").html());g=a.find("button").addClass("hide");if("Submitting"===e||"Submitted"===
e||"Running"===e)g.removeClass("hide"),g.text("Refresh status"),g.on("click",function(){d.updateSubmissionStatus($("#competitionId").val(),b.id,this)});"Failed"!==e&&"Cancelled"!==e||a.find("a").removeClass("hide")}$(b).after(a)}};d.updateSubmissionStatus=function(a,b,e){$(e).parents(".submission_details").find(".preloader-handler").append("<div class='competitionPreloader'></div>").children().css({top:"25px",display:"block"});$.ajax({type:"GET",url:"/api/competition/"+a+"/submission/"+b,cache:!1,
success:function(f){$("#user_results #"+b).find(".statusName").html(d.getSubmissionStatus(f.status));if("finished"===f.status)$("#user_results #"+b+"input:hidden").val("1"),1==$("#phasestate").val()?($(e).addClass("leaderBoardSubmit"),$(e).text("Submit to Leaderboard"),$(e).on("click",function(){c(a,b,$("#cstoken").val(),$(e))})):$(e).addClass("hide");else if("failed"===f.status||"cancelled"===f.status)$(e).addClass("hide"),$(e).parent().parent().find("a").removeClass("hide");$(".competitionPreloader").hide()},
error:function(a,b,e){}})}})(Competition||(Competition={}));var prTable;
