...
a.appendChild(f)},b=function(a,b){var c=document.createElement("img"),f=[b.getUrl().replace("api/bundles/content","api/bundles/filecontent"),b.getFullName()].join("/");c.setAttribute("src",f);c.setAttribute("style","max-width:900px");return[c]},k=function(a,b){var c=document.createElement("iframe"),f=[b.getUrl().replace("api/bundles/content","api/bundles/filecontent"),b.getFullName()].join("/");c.setAttribute("src",f);c.setAttribute("style","width:100%; min-height:"+Math.max(200,Math.min(900,screen.height/
2))+"px;");return[c]},l={txt:function(a,b){var c=document.createElement("pre");c.setAttribute("class","bundle__file_pre");c.textContent=b.getData();return[c]},htm:k,html:k,jpg:b,png:b},n=function(a,b){var c=b.getName(),f="";0<c.indexOf(".")&&(f=c.toLowerCase().split(".").pop());c=l[f];void 0===c&&(c=l.txt);a.setAttribute("class","col");c(a,b).forEach(function(b){a.appendChild(b)})};return{tableDecorations:"bundle__file_view expanded",rowDecorations:"row",numRows:a,numCols:2,render:function(a,b,c){if(!1===
e.isRootNode()&&0>=a)0==b?f(c):g(c);else if(!0===e.isLeafNode())0==b?h(c):n(c,e);else{var d=0;!1===e.isRootNode()&&(d=1);a=e.getChildren()[a-d];0==b?h(c,a):m(c,a)}}}};c.getMetadataTableModel=function(c){var d=[],a;for(a in c.metadata)console.log(a),"description"==a&&d.push([a,c.metadata[a]]);return{tableDecorations:"bundle__meta_table",rowDecorations:"row",numRows:d.length,numCols:2,render:function(a,c,g){0==c?g.setAttribute("class","col bundle__meta_type"):g.setAttribute("class","col");g.textContent=
d[a][c]}}};c.getElementType=function(c,d){return void 0!==c?c:d};c.renderTable=function(e,d){var a=c.getElementType(d.tableElementType,"table"),b=c.getElementType(d.rowElementType,"tr"),f=c.getElementType(d.columnElementType,"td"),a=document.createElement(a);void 0!==d.tableDecorations&&a.setAttribute("class",d.tableDecorations);for(var g=0;g<d.numRows;g++){var h=document.createElement(b);if(0<d.numCols)for(var m=0;m<d.numCols;m++){var k=document.createElement(f);d.render(g,m,k);h.appendChild(k)}else d.render(g,
h);a.appendChild(h)}void 0!=e.firstChild&&e.removeChild(e.firstChild);e.appendChild(a)};c.prototype.render=function(e){var d=$(this.template.cloneNode(!0));d.find(".bundle-name").text(e.metadata.name);d.find(".bundle-icon-sm").removeClass("bundle-icon-sm").addClass("bundle-icon-sm--"+e.bundle_type+"--"+e.state);d.find(".bundle-uuid").text(e.uuid);d.find(".bundle-link").attr("href","/bundles/"+e.uuid);d.find(".bundle-download").on("click",function(a){a.preventDefault();console.log(a);console.log(b.get(0));
root=new BundleContentNode("/api/bundles/content",e.uuid);console.log(root)});var a=d.find(".bundle-meta-view-container").get(0);c.renderTable(a,c.getMetadataTableModel(e));var a=d.find(".bundle__expand_button"),b=d.find(".bundle-file-view-container");a.on("click",function(a){var g=$(a.target);if(g.hasClass("expanded"))b.removeClass("expanded"),b.children().removeClass("expanded"),g.removeClass("expanded"),g.html('SHOW BUNDLE CONTENT<img src="/static/img/expand-arrow.png" alt="More">');else{if(void 0==
b.get(0).firstChild){var h=new BundleContentNode("/api/bundles/content",e.uuid);c.loadContentAsync(b.get(0),h)}else b.children().addClass("expanded");b.addClass("expanded");g.addClass("expanded");g.html('HIDE BUNDLE CONTENT<img src="/static/img/expand-arrow.png" alt="Less">')}a.preventDefault()});return d.get(0)};return c}(),WorkshhetDirectiveRenderer=function(){return function(){var c=this;c.display="inline";c.bundleBlock=null;c.tableDirective=[];c.applyDirective=function(e,d){switch(d[1].directive){case "display":c.display=
...
e,d){this.renderer=e;if(d&&d.items&&Array.isArray(d.items)){var a=this;e=d.name;var b=d.items.filter(function(a){return"title"===a[2]});0<b.length&&(e=markdown.toHTML("#"+b[0][1]).replace(/^<h1>/,"").replace(/<\/h1>$/,""));$(".worksheet-icon").html(e);$(".worksheet-author").html("by "+d.owner);var f=new WorkshhetDirectiveRenderer,g="";d.items.push([null,"eof",null]);d.items.forEach(function(b,d,e){"directive"!=b[2]&&f.applyPendingDirectives(c);if("markup"!=b[2]&&0<g.length){var l=document.createElement("div");
l.innerHTML=markdown.toHTML(g);c.appendChild(l);g=""}switch(b[2]){case "markup":g+=b[1]+"\n\r";break;case "bundle":c.appendChild(a.renderer.render(b[0]));break;case "directive":e.slice(d+1).forEach(function(a){"bundle"==a[2]&&(f.bundleBlock=a[0])}),f.applyDirective(c,b)}});MathJax.Hub.Queue(["Typeset",MathJax.Hub,c.id]);MathJax.Hub.Queue(function(){for(var a=MathJax.Hub.getAllJax(),b=0;b<a.length;b+=1)$(a[b].SourceElement().parentNode).addClass("coda-jax")})}}}();var Competition;
...
"delete");request=$.ajax({url:a,type:h,datatype:"text",data:{csrfmiddlewaretoken:c},success:function(a,c,f){(a="post"==h)?(c=$("#user_results td.status"),c.removeClass("submitted"),c.addClass("not_submitted"),c.html(""),c=$("#"+b+" td.status"),c.addClass("submitted"),c.html("<i class='fi-check'></i>"),$("#user_results button.leaderBoardRemove").each(function(a){e($(this),!1)})):(c=$("#"+b+" td.status"),c.removeClass("submitted"),c.addClass("not_submitted"),c.html(""));e(d,a)},error:function(a,b,c){alert("An error occurred. Please try again or report the issue")},
beforeSend:function(a){a.setRequestHeader("X-CSRFToken",c)}})}c.invokePhaseButtonOnOpen=function(a){var b=$("#"+a+" .btn.selected")[0];void 0===b&&(b=$("#"+a+" .btn.active")[0],void 0===b&&(b=$("#"+a+" .btn")[0]));b.click()};c.getPhaseSubmissions=function(a,b,f){$(".competition_submissions").html("").append("<div class='competitionPreloader'></div>").children().css({top:"200px",display:"block"});$.ajax({type:"GET",url:"/competitions/"+a+"/submissions/"+b,cache:!1,success:function(b){$(".competition_submissions").html("").append(b);
...
$(b).attr("data-exception")+"</pre>");var f=$("#phasestate").val(),g=$(b).find("input[name='state']").val();if(1==f&&1==g){var h=a.find("button");h.removeClass("hide");var f=$(b).find(".status").hasClass("submitted"),m=$("#competitionId").val();e(h,f);h.on("click",function(){d(m,b.id,$("#cstoken").val(),h)})}else{f=$.trim($(b).find(".statusName").html());h=a.find("button").addClass("hide");if("Submitting"===f||"Submitted"===f||"Running"===f)h.removeClass("hide"),h.text("Refresh status"),h.on("click",
...
if("finished"===e.status)$("#user_results #"+b+"input:hidden").val("1"),1==$("#phasestate").val()?(0<$("#disallow_leaderboard_modifying").length?($(f).text("Leaderboard modifying disallowed").attr("disabled","disabled"),0<$("#forced_to_leaderboard").length&&($(".fi-check").remove(),$($("#"+b+" td")[4]).html('<i class="fi-check"></i>'))):0==$("#forced_to_leaderboard").length?($(f).addClass("leaderBoardSubmit"),$(f).text("Submit to Leaderboard")):($(".fi-check").remove(),$($("#"+b+" td")[4]).html('<i class="fi-check"></i>'),
...
error:function(a,b,c){}})};c.initialize=function(){$(document).ready(function(){$("#participate_form").submit(function(a){a.preventDefault();if($("#participateButton").hasClass("disabledStatus"))return!1;$("#result").html("");a=$(this).serialize();var f=$("#competitionId").val();request=$.ajax({url:"/api/competition/"+f+"/participate/",type:"post",dataType:"text",data:a,success:function(a,b,d){$(".content form").replaceWith(c.displayRegistrationStatus(f))},error:function(a,b,c){alert("There was a problem registering for this competition.")}});
return!1});$("#submissions_phase_buttons .btn").each(function(a,f){$(this).click(function(){var a=$.trim($(this).attr("id").replace("submissions_phase_",""));$("#submissions_phase_buttons .btn").removeClass("selected");$(this).addClass("selected");var b=$("#competitionId").val(),f=$("#cstoken").val();c.getPhaseSubmissions(b,a,f)})});$("a[href='#participate-submit_results']").click(function(a){c.invokePhaseButtonOnOpen("submissions_phase_buttons")});$("#results_phase_buttons .btn").each(function(a,
f){$(this).click(function(){var a=$.trim($(this).attr("id").replace("results_phase_",""));$("#results_phase_buttons .btn").removeClass("selected");$(this).addClass("selected");var b=$("#competitionId").val();c.getPhaseResults(b,a)})});$("#Results").click(function(a){c.invokePhaseButtonOnOpen("results_phase_buttons")});$(".top-bar-section ul > li").removeClass("active");$("#liCompetitions").addClass("active");$(".my_managing .competition-tile #competition-actions").each(function(a,c){$(this).children("#competition-publish-button").click(function(){var a=
$(this).parent()[0];request=$.ajax({url:$(this)[0].value,success:function(b,c,f){console.log("Published competition.");$(a).children("#competition-publish-button").hide();$(a).children("#competition-delete-button").hide();$(a).children("#competition-unpublish-button").show()},error:function(a,b,c){a=$.parseJSON(a.responseJSON);a.error&&alert(a.error);console.log("Error publishing competition!")}})});$(this).children("#competition-unpublish-button").click(function(){});"True"==$(this)[0].getAttribute("published")?
($(this).children("#competition-delete-button").hide(),$(this).children("#competition-publish-button").hide(),$(this).children("#competition-unpublish-button").show()):($(this).children("#competition-delete-button").show(),$(this).children("#competition-publish-button").show(),$(this).children("#competition-unpublish-button").hide())});var a=window.location.href;void 0!==a&&(null!==a.match(/#participate-submit_results$/i)?c.invokePhaseButtonOnOpen("submissions_phase_buttons"):null!==a.match(/#results$/i)&&
c.invokePhaseButtonOnOpen("results_phase_buttons"))})}})(Competition||(Competition={}));var csrftoken;function csrfSafeMethod(c){return/^(GET|HEAD|OPTIONS|TRACE)$/.test(c)}function sameOrigin(c){var e="//"+document.location.host,d=document.location.protocol+e;return c==d||c.slice(0,d.length+1)==d+"/"||c==e||c.slice(0,e.length+1)==e+"/"||!/^(\/\/|http:|https:).*/.test(c)}$(function(){csrftoken=$.cookie("csrftoken");$.ajaxSetup({beforeSend:function(c,e){!csrfSafeMethod(e.type)&&sameOrigin(e.url)&&c.setRequestHeader("X-CSRFToken",csrftoken)}})});var CodaLab;
...
$("#details").html("Please select a valid file. Only ZIP files are accepted."):$("#details").html("The files that you selected is too large. There is a 1GB size limit.")),$("#uploadButton").removeClass("disabled"))},uploadProgress:function(a,b,c){b=100*b/c;$("#details").html("Uploading file <em>"+a.name+"</em>: "+b.toFixed(0)+"% complete.")},uploadError:function(a){$("#details").html("There was an error uploading the file. Please try again.");$("#uploadButton").removeClass("disabled")},uploadSuccess:function(a,
