...
uploadSuccess:function(b,f){$("#details").html("Creating new submission...");var d=$("#submission_description_textarea").val(),h=$("#submission_method_name").val(),e=$("#submission_method_description").val();$("#submission_description_textarea").val("");$.ajax({url:"/api/competition/"+a+"/submission?description="+encodeURIComponent(d)+"&method_name="+encodeURIComponent(h)+"&method_description="+encodeURIComponent(e),type:"post",cache:!1,data:{id:f,name:b.name,type:b.type,size:b.size}}).done(function(a){$("#details").html("");
$("#user_results tr.noData").remove();$("#user_results").append(c.displayNewSubmission(a,d,h,e));$("#user_results #"+a.id+" .glyphicon-plus").on("click",function(){c.showOrHideSubmissionDetails(this)});$("#fileUploadButton").removeClass("disabled");$("#user_results #"+a.id+" .glyphicon-plus").click()}).fail(function(a){var b="An unexpected error occurred.";403==a.status&&(b=a.responseJSON.detail);$("#details").html(b);$("#fileUploadButton").removeClass("disabled")})}});$("#user_results .glyphicon-plus").on("click",
...
function(a,b,f,d){var g=$("#submission_details_template #submission_row_template tr").clone();$(g).attr("id",a.id.toString());$(g).addClass(c.oddOrEven(a.submission_number));b&&$(g).attr("data-description",b);f&&$(g).attr("data-method-name",f);d&&$(g).attr("data-method-description",d);$(g).children().each(function(b){switch(b){case 0:"finished"===a.status&&($(this).val("1"),0<$("#forced_to_leaderboard").length&&($(".glyphicon-ok").remove(),$($(g).children("td")[4]).html('<span class="glyphicon glyphicon-ok"></span>')));
break;case 1:$(this).html(a.submission_number.toString());break;case 2:$(this).html(a.filename);break;case 3:var f=function(a){a=a.toString();1==a.length&&(a="0"+a);return a},d=new Date(a.submitted_at);b=d.getDate().toString()+"/"+d.getMonth().toString()+"/"+d.getFullYear();var e=d.getHours().toString(),h=f(d.getMinutes()),f=f(d.getSeconds());$(this).html(b+" "+e+":"+h+":"+f);break;case 4:$(this).html(c.getSubmissionStatus(a.status))}});return g};c.oddOrEven=function(a){return a&1?"odd":"even"};c.getSubmissionStatus=
...
$(b).attr("data-method-description")+"</pre>");$(b).attr("data-exception")&&a.find(".traceback").html("Error: <br><pre>"+$(b).attr("data-exception")+"</pre>");var f=$("#phasestate").val(),h=$(b).find("input[name='state']").val();if(1==f&&1==h){var g=a.find("button");g.removeClass("hide");var f=$(b).find(".status").hasClass("submitted"),m=$("#competitionId").val();e(g,f);g.on("click",function(){d(m,b.id,$("#cstoken").val(),g)})}else{f=$.trim($(b).find(".statusName").html());g=a.find("button").addClass("hide");
