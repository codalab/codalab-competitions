...
$(f).removeClass("leaderBoardSubmit"),$(f).addClass("leaderBoardRemove"),$(f).text("Remove from Leaderboard")),$(f).unbind("click").off("click").on("click",function(){d(a,c,$("#cstoken").val(),$(f))})):$(f).addClass("hide");else if("failed"===e.status||"cancelled"===e.status)$(f).addClass("hide"),$(f).parent().parent().find("a").removeClass("hide");$(".competitionPreloader").hide()},error:function(a,b,c){}})};b.initialize=function(){$(document).ready(function(){$("#participate_form").submit(function(a){a.preventDefault();
...
$("#uploadButton").removeClass("disabled")})}})}})(b.Competitions||(b.Competitions={}))})(CodaLab||(CodaLab={}));angular.module("codalab.services",[]);angular.module("codalab.controllers",[]);angular.module("codalab.directives",[]);
