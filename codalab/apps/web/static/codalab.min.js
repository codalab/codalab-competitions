...
e.isRootNode()&&0>=a)0==c?f(b):h(b);else if(!0===e.isLeafNode())0==c?g(b):n(b,e);else{var d=0;!1===e.isRootNode()&&(d=1);a=e.getChildren()[a-d];0==c?g(b,a):m(b,a)}}}};b.getMetadataTableModel=function(b){var d=[],a;for(a in b.metadata)console.log(a),"description"==a&&d.push([a,b.metadata[a]]);return{tableDecorations:"bundle__meta_table",rowDecorations:"row",numRows:d.length,numCols:2,render:function(a,b,h){0==b?h.setAttribute("class","col bundle__meta_type"):h.setAttribute("class","col");h.textContent=
d[a][b]}}};b.getElementType=function(b,d){return void 0!==b?b:d};b.renderTable=function(e,d){var a=b.getElementType(d.tableElementType,"table"),c=b.getElementType(d.rowElementType,"tr"),f=b.getElementType(d.columnElementType,"td"),a=document.createElement(a);void 0!==d.tableDecorations&&a.setAttribute("class",d.tableDecorations);for(var h=0;h<d.numRows;h++){var g=document.createElement(c);if(0<d.numCols)for(var m=0;m<d.numCols;m++){var k=document.createElement(f);d.render(h,m,k);g.appendChild(k)}else d.render(h,
...
break;case 3:var d=function(a){a=a.toString();1==a.length&&(a="0"+a);return a},g=new Date(a.submitted_at);f=g.getDate().toString()+"/"+g.getMonth().toString()+"/"+g.getFullYear();var e=g.getHours().toString(),k=d(g.getMinutes()),d=d(g.getSeconds());$(this).html(f+" "+e+":"+k+":"+d);break;case 4:$(this).html(b.getSubmissionStatus(a.status))}});return c};b.oddOrEven=function(a){return a&1?"odd":"even"};b.getSubmissionStatus=function(a){var c="Unknown";"submitting"===a?c="Submitting":"submitted"===a?
...
$(this).attr("href").replace("_",c.id))});var f=$("#phasestate").val(),h=$(c).find("input[name='state']").val();if(1==f&&1==h){var g=a.find("button");g.removeClass("hide");var f=$(c).find(".status").hasClass("submitted"),m=$("#competitionId").val();e(g,f);g.on("click",function(){d(m,c.id,$("#cstoken").val(),g)})}else{f=$.trim($(c).find(".statusName").html());g=a.find("button").addClass("hide");if("Submitting"===f||"Submitted"===f||"Running"===f)g.removeClass("hide"),g.text("Refresh status"),g.on("click",
...
success:function(c,b,d){console.log("Published competition.");$(a).children("#competition-publish-button").hide();$(a).children("#competition-delete-button").hide();$(a).children("#competition-unpublish-button").show()},error:function(a,c,b){a=$.parseJSON(a.responseJSON);a.error&&alert(a.error);console.log("Error publishing competition!")}})});$(this).children("#competition-unpublish-button").click(function(){});"True"==$(this)[0].getAttribute("published")?($(this).children("#competition-delete-button").hide(),
...
c.name);a.setRequestHeader("x-ms-meta-size",c.size.toString())},success:function(a,b){e.options.uploadSuccess(c,e.state.trackingId);var d=1+e.state.fileIndex;d<e.state.selection.files.length?e.beginUpload(e.state.selection,d):e.setupFileInput()},error:function(a,b,c){e.options.uploadError({kind:"write-error",jqXHR:a,file:e.getCurrentFile()})}})};b.isSupported=function(){return window.File&&window.FileReader&&window.FileList&&window.Blob?!0:!1};return b}();b.FileUploader=e;(function(b){b.CreateReady=
...
(b.Competitions={}))})(CodaLab||(CodaLab={}));//# sourceMappingURL=/static/codalab.min.map
