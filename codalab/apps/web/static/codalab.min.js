...
$(this).attr("href").replace("_",c.id))});$(c).attr("data-exception")&&a.find(".traceback").html("Error: <br><pre>"+$(c).attr("data-exception")+"</pre>");var f=$("#phasestate").val(),g=$(c).find("input[name='state']").val();if(1==f&&1==g){var h=a.find("button");h.removeClass("hide");var f=$(c).find(".status").hasClass("submitted"),m=$("#competitionId").val();e(h,f);h.on("click",function(){d(m,c.id,$("#cstoken").val(),h)})}else{f=$.trim($(c).find(".statusName").html());h=a.find("button").addClass("hide");
if("Submitting"===f||"Submitted"===f||"Running"===f)h.removeClass("hide"),h.text("Refresh status"),h.on("click",function(){b.updateSubmissionStatus($("#competitionId").val(),c.id,this)});"Failed"!==f&&"Cancelled"!==f||a.find("a").removeClass("hide")}$(c).after(a)}};b.updateSubmissionStatus=function(a,c,f){$(f).parents(".submission_details").find(".preloader-handler").append("<div class='competitionPreloader'></div>").children().css({top:"25px",display:"block"});$.ajax({type:"GET",url:"/api/competition/"+
a+"/submission/"+c,cache:!1,success:function(g){$("#user_results #"+c).find(".statusName").html(b.getSubmissionStatus(g.status));if("finished"===g.status)$("#user_results #"+c+"input:hidden").val("1"),1==$("#phasestate").val()?(0<$("#disallow_leaderboard_modifying").length?($(f).text("Leaderboard modifying disallowed").attr("disabled","disabled"),0<$("#forced_to_leaderboard").length&&($(".fi-check").remove(),$($("#"+c+" td")[4]).html('<i class="fi-check"></i>'))):0==$("#forced_to_leaderboard").length?
($(f).addClass("leaderBoardSubmit"),$(f).text("Submit to Leaderboard")):($(".fi-check").remove(),$($("#"+c+" td")[4]).html('<i class="fi-check"></i>'),$(f).removeClass("leaderBoardSubmit"),$(f).addClass("leaderBoardRemove"),$(f).text("Remove from Leaderboard")),$(f).unbind("click").off("click").on("click",function(){d(a,c,$("#cstoken").val(),$(f))})):$(f).addClass("hide");else if("failed"===g.status||"cancelled"===g.status)$(f).addClass("hide"),$(f).parent().parent().find("a").removeClass("hide"),
g.exception_details&&$("a[href='traceback/"+c+"/']").parent().html("Error: <br><pre>"+g.exception_details+"</pre>");$(".competitionPreloader").hide()},error:function(a,c,b){}})};b.initialize=function(){$(document).ready(function(){$("#participate_form").submit(function(a){a.preventDefault();if($("#participateButton").hasClass("disabledStatus"))return!1;$("#result").html("");a=$(this).serialize();var f=$("#competitionId").val();request=$.ajax({url:"/api/competition/"+f+"/participate/",type:"post",
dataType:"text",data:a,success:function(a,c,d){$(".content form").replaceWith(b.displayRegistrationStatus(f))},error:function(a,c,b){alert("There was a problem registering for this competition.")}});return!1});$("#submissions_phase_buttons .button").each(function(a,f){$(this).click(function(){var a=$.trim($(this).attr("id").replace("submissions_phase_",""));$("#submissions_phase_buttons .button").removeClass("selected");$(this).addClass("selected");var c=$("#competitionId").val(),f=$("#cstoken").val();
b.getPhaseSubmissions(c,a,f)})});$("a[href='#participate-submit_results']").click(function(a){b.invokePhaseButtonOnOpen("submissions_phase_buttons")});$("#results_phase_buttons .button").each(function(a,f){$(this).click(function(){var a=$.trim($(this).attr("id").replace("results_phase_",""));$("#results_phase_buttons .button").removeClass("selected");$(this).addClass("selected");var c=$("#competitionId").val();b.getPhaseResults(c,a)})});$("#Results").click(function(a){b.invokePhaseButtonOnOpen("results_phase_buttons")});
$(this).foundation("section","reflow");$(".top-bar-section ul > li").removeClass("active");$("#liCompetitions").addClass("active");$(".my_managing .competition-tile #competition-actions").each(function(a,b){$(this).children("#competition-publish-button").click(function(){var a=$(this).parent()[0];request=$.ajax({url:$(this)[0].value,success:function(c,b,f){console.log("Published competition.");$(a).children("#competition-publish-button").hide();$(a).children("#competition-delete-button").hide();$(a).children("#competition-unpublish-button").show()},
error:function(a,c,b){a=$.parseJSON(a.responseJSON);a.error&&alert(a.error);console.log("Error publishing competition!")}})});$(this).children("#competition-unpublish-button").click(function(){});"True"==$(this)[0].getAttribute("published")?($(this).children("#competition-delete-button").hide(),$(this).children("#competition-publish-button").hide(),$(this).children("#competition-unpublish-button").show()):($(this).children("#competition-delete-button").show(),$(this).children("#competition-publish-button").show(),
...
$("#details").html("Please select a valid file. Only ZIP files are accepted."):$("#details").html("The files that you selected is too large. There is a 1GB size limit.")),$("#uploadButton").removeClass("disabled"))},uploadProgress:function(a,c,b){c=100*c/b;$("#details").html("Uploading file <em>"+a.name+"</em>: "+c.toFixed(0)+"% complete.")},uploadError:function(a){$("#details").html("There was an error uploading the file. Please try again.");$("#uploadButton").removeClass("disabled")},uploadSuccess:function(a,
