...
"eof",null]);d.items.forEach(function(c,d,e){"directive"!=c[2]&&f.applyPendingDirectives(b);if(("markup"!=c[2]||"markup"==c[2]&&""==c[1])&&0<h.length){var m=document.createElement("div");m.innerHTML=markdown.toHTML(h);b.appendChild(m);h="";MathJax.Hub.Queue(["Typeset",MathJax.Hub,m])}switch(c[2]){case "markup":""!=c[1]&&(h+=c[1]+" ");break;case "bundle":b.appendChild(a.renderer.render(c[0]));break;case "directive":e.slice(d+1).forEach(function(a){"bundle"==a[2]&&(f.bundleBlock=a[0])}),f.applyDirective(b,
c)}})}}}();var Competition;
