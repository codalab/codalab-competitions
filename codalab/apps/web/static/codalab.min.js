...
void 0!==b&&f.setAttribute("class",b.isLeafNode()?"fi-page":"fi-folder");a.appendChild(f)},n=function(a,b){if(b.isLeafNode()){a.setAttribute("class","col");var f=document.createElement("a");f.textContent=b.getName();f.setAttribute("href","");f.onclick=function(a){c.loadFileContentAsync(d,b);a.preventDefault()}}else a.setAttribute("class","col bundle-item-type-folder"),f=document.createElement("a"),f.textContent=b.getName(),f.setAttribute("href",""),f.onclick=function(a){c.loadContentAsync(d,b);a.preventDefault()};
a.appendChild(f)},b=function(a,b){var f=document.createElement("img"),c=[b.getUrl().replace("api/bundles/content","api/bundles/filecontent"),b.getFullName()].join("/");f.setAttribute("src",c);f.setAttribute("style","max-width:900px");return[f]},l=function(a,b){var f=document.createElement("iframe"),c=[b.getUrl().replace("api/bundles/content","api/bundles/filecontent"),b.getFullName()].join("/");f.setAttribute("src",c);f.setAttribute("style","width:100%; min-height:"+Math.max(200,Math.min(900,screen.height/
2))+"px;");return[f]},m={txt:function(a,b){var f=document.createElement("pre");f.setAttribute("class","bundle__file_pre");f.textContent=b.getData();return[f]},htm:l,html:l,jpg:b,png:b},p=function(a,b){var f=b.getName(),c="";0<f.indexOf(".")&&(c=f.toLowerCase().split(".").pop());f=m[c];void 0===f&&(f=m.txt);a.setAttribute("class","col");f(a,b).forEach(function(b){a.appendChild(b)})};return{tableDecorations:"bundle__file_view expanded",rowDecorations:"row",numRows:a,numCols:2,render:function(a,b,c){if(!1===
e.isRootNode()&&0>=a)0==b?f(c):h(c);else if(!0===e.isLeafNode())0==b?g(c):p(c,e);else{var d=0;!1===e.isRootNode()&&(d=1);a=e.getChildren()[a-d];0==b?g(c,a):n(c,a)}}}};c.getMetadataTableModel=function(c){var d=[],a;for(a in c.metadata)console.log(a),"description"==a&&d.push([a,c.metadata[a]]);return{tableDecorations:"bundle__meta_table",rowDecorations:"row",numRows:d.length,numCols:2,render:function(a,f,c){0==f?c.setAttribute("class","col bundle__meta_type"):c.setAttribute("class","col");c.textContent=
...
"delete");request=$.ajax({url:a,type:g,datatype:"text",data:{csrfmiddlewaretoken:c},success:function(a,c,f){(a="post"==g)?(c=$("#user_results td.status"),c.removeClass("submitted"),c.addClass("not_submitted"),c.html(""),c=$("#"+b+" td.status"),c.addClass("submitted"),c.html('<span class="glyphicon glyphicon-ok"></span>'),$("#user_results .leaderboard_button.leaderBoardRemove").each(function(a){e($(this),!1)})):(c=$("#"+b+" td.status"),c.removeClass("submitted"),c.addClass("not_submitted"),c.html(""));
...
return!1};c.updateSubmissionDescription=function(a){a.preventDefault();var b=this;b.blur();a=$(this).attr("submission-id");var c=$(this).parent().find('textarea[name="updated_description"]').val()||"";$.post("/competitions/update_description/"+a,{updated_description:c}).success(function(){$(b).parent().find(".submission_description").html("<b>Description:</b> <br><pre>"+c+"</pre>");$(b).parent().find('textarea[name="updated_description"]').val("")}).error(function(){alert("Error updating description, is your Internet connection working?")})};
c.showOrHideSubmissionDetails=function(a){var b=$(a).parents("tr")[0];if($(a).hasClass("glyphicon-minus"))$(a).removeClass("glyphicon-minus"),$(a).addClass("glyphicon-plus"),$(b).next("tr.trDetails").remove();else{$(a).removeClass("glyphicon-plus");$(a).addClass("glyphicon-minus");a=$("#submission_details_template .trDetails").clone();a.find(".tdDetails").attr("colspan",b.cells.length);a.find("a").each(function(a){$(this).attr("href",$(this).attr("href").replace("_",b.id))});void 0!==$(b).attr("data-description")&&
a.find(".submission_description").html("<b>Description:</b> <br><pre>"+$(b).attr("data-description")+"</pre>");$(b).attr("data-team-name")&&a.find(".submission_team_name").html("<b>Team name:</b> "+$(b).attr("data-team-name"));$(b).attr("data-method-name")&&a.find(".submission_method_name").html("<b>Method name:</b> "+$(b).attr("data-method-name"));$(b).attr("data-method-description")&&a.find(".submission_method_description").html("<b>Method description:</b><br><pre>"+$(b).attr("data-method-description")+
"</pre>");$(b).attr("data-project-url")&&a.find(".submission_project_url").html('<b>Project URL:</b> <a href="'+$(b).attr("data-project-url")+'">'+$(b).attr("data-project-url")+"</a>");$(b).attr("data-publication-url")&&a.find(".submission_publication_url").html('<b>Publication URL:</b> <a href="'+$(b).attr("data-publication-url")+'">'+$(b).attr("data-publication-url")+"</a>");$(b).attr("data-bibtex")&&a.find(".submission_bibtex").html("<b>Bibtex:</b><br><pre>"+$(b).attr("data-bibtex")+"</pre>");
$(b).attr("data-organization-or-affiliation")&&a.find(".submission_organization_or_affiliation").html("<b>Organization/affiliation:</b> "+$(b).attr("data-organization-or-affiliation"));$(b).attr("data-exception")&&a.find(".traceback").html("Error: <br><pre>"+$(b).attr("data-exception")+"</pre>");var f=$(b).attr("data-is-public")?"private":"public";a.find(".public_link").html("Make your submission "+f);a.find(".public_link").click(c.toggleSubmissionPublic);a.find(".public_link").attr("submission-id",
b.id);a.find(".update_description").click(c.updateSubmissionDescription);a.find(".update_description").attr("submission-id",b.id);var f=$("#phasestate").val(),h=$(b).find("input[name='state']").val();if(1==f&&1==h){var g=a.find(".leaderboard_button");g.removeClass("hide");var f=$(b).find(".status").hasClass("submitted"),n=$("#competitionId").val();e(g,f);g.on("click",function(){d(n,b.id,$("#cstoken").val(),g)})}else{f=$.trim($(b).find(".statusName").html());g=a.find(".leaderboard_button").addClass("hide");
if("Submitting"===f||"Submitted"===f||"Running"===f)g.removeClass("hide"),g.text("Refresh status"),g.on("click",function(){c.updateSubmissionStatus($("#competitionId").val(),b.id,this)});"Failed"!==f&&"Cancelled"!==f||a.find("a").removeClass("hide")}$(b).after(a)}};c.updateSubmissionStatus=function(a,b,f){$(f).parents(".submission_details").find(".preloader-handler").append("<div class='competitionPreloader'></div>").children().css({top:"25px",display:"block"});$.ajax({type:"GET",url:"/api/competition/"+
...
($(f).addClass("leaderBoardSubmit"),$(f).text("Submit to Leaderboard")):($(".glyphicon-ok").remove(),$($("#"+b+" td")[4]).html('<span class="glyphicon glyphicon-ok"></span>'),$(f).removeClass("leaderBoardSubmit"),$(f).addClass("leaderBoardRemove"),$(f).text("Remove from Leaderboard")),$(f).unbind("click").off("click").on("click",function(){d(a,b,$("#cstoken").val(),$(f))})):$(f).addClass("hide");else if("failed"===e.status||"cancelled"===e.status)$(f).addClass("hide"),$(f).parent().parent().find("a").removeClass("hide"),
e.exception_details&&$('a[href="traceback/'+b+'/"]').parent().html("Error: <br><pre>"+e.exception_details+"</pre>");$(".competitionPreloader").hide()},error:function(a,b,c){}})};c.initialize=function(){$(document).ready(function(){$("#participate_form").submit(function(a){a.preventDefault();if($("#participateButton").hasClass("disabledStatus"))return!1;$("#result").html("");a=$(this).serialize();var b=$("#competitionId").val();request=$.ajax({url:"/api/competition/"+b+"/participate/",type:"post",
dataType:"text",data:a,success:function(a,d,e){$(".content form").replaceWith(c.displayRegistrationStatus(b))},error:function(a,b,c){alert("There was a problem registering for this competition.")}});return!1});$("#submissions_phase_buttons .btn").each(function(a,b){$(this).click(function(){var a=$.trim($(this).attr("id").replace("submissions_phase_",""));$("#submissions_phase_buttons .btn").removeClass("selected");$(this).addClass("selected");var b=$("#competitionId").val(),d=$("#cstoken").val();
c.getPhaseSubmissions(b,a,d)})});$("#results_phase_buttons .btn").each(function(a,b){$(this).click(function(){var a=$.trim($(this).attr("id").replace("results_phase_",""));$("#results_phase_buttons .btn").removeClass("selected");$(this).addClass("selected");var b=$("#competitionId").val();c.getPhaseResults(b,a)})});$(".top-bar-section ul > li").removeClass("active");$("#liCompetitions").addClass("active");$("#competition-publish-button").click(function(a){a.preventDefault();var b=$(this).parent()[0];
request=$.ajax({url:$(this).attr("href"),success:function(a,c,d){console.log("Published competition.");$(b).children("#competition-publish-button").hide();$(b).children("#competition-delete-button").hide();$(b).children("#competition-unpublish-button").show()},error:function(a,b,c){a=$.parseJSON(a.responseJSON);a.error&&alert(a.error);console.log("Error publishing competition!")}})});$("#competition-unpublish-button").click(function(a){a.preventDefault();var b=$(this).parent()[0];request=$.ajax({url:$(this).attr("href"),
success:function(a,c,d){console.log("Unpublished competition.");$(b).children("#competition-publish-button").show();$(b).children("#competition-delete-button").show();$(b).children("#competition-unpublish-button").hide()},error:function(a,b,c){console.log("Error unpublishing competition!")}})});$("#my_managing .competition-tile .competition-actions").each(function(a,b){"True"==$(this)[0].getAttribute("published")?($(this).find("#competition-delete-button").hide(),$(this).find("#competition-publish-button").hide(),
$(this).find("#competition-unpublish-button").show()):($(this).find("#competition-delete-button").show(),$(this).find("#competition-publish-button").show(),$(this).find("#competition-unpublish-button").hide())})})}})(Competition||(Competition={}));var csrftoken;function csrfSafeMethod(c){return/^(GET|HEAD|OPTIONS|TRACE)$/.test(c)}function sameOrigin(c){var e="//"+document.location.host,d=document.location.protocol+e;return c==d||c.slice(0,d.length+1)==d+"/"||c==e||c.slice(0,e.length+1)==e+"/"||!/^(\/\/|http:|https:).*/.test(c)}$(function(){csrftoken=$.cookie("csrftoken");$.ajaxSetup({beforeSend:function(c,e){!csrfSafeMethod(e.type)&&sameOrigin(e.url)&&c.setRequestHeader("X-CSRFToken",csrftoken)}})});var CodaLab;
