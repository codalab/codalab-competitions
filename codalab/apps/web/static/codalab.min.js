...
if("finished"===e.status)$("#user_results #"+c+"input:hidden").val("1"),1==$("#phasestate").val()?0==$("#forced_to_leaderboard").length?($(f).addClass("leaderBoardSubmit"),$(f).text("Submit to Leaderboard"),$(f).on("click",function(){d(a,c,$("#cstoken").val(),$(f))})):($(".fi-check").remove(),$($("#"+c+" td")[4]).html('<i class="fi-check"></i>'),$(f).removeClass("leaderBoardSubmit"),$(f).addClass("leaderBoardRemove"),$(f).text("Remove from Leaderboard")):$(f).addClass("hide");else if("failed"===e.status||
"cancelled"===e.status)$(f).addClass("hide"),$(f).parent().parent().find("a").removeClass("hide");$(".competitionPreloader").hide()},error:function(a,b,c){}})};b.initialize=function(){$(document).ready(function(){$("#participate_form").submit(function(a){a.preventDefault();if($("#participateButton").hasClass("disabledStatus"))return!1;$("#result").html("");a=$(this).serialize();var d=$("#competitionId").val();request=$.ajax({url:"/api/competition/"+d+"/participate/",type:"post",dataType:"text",data:a,
