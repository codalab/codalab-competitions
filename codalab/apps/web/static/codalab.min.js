...
void 0!==b&&c.setAttribute("class",b.isLeafNode()?"fi-page":"fi-folder");a.appendChild(c)},m=function(a,b){if(b.isLeafNode()){a.setAttribute("class","col");var f=document.createElement("a");f.textContent=b.getName();f.setAttribute("href","");f.onclick=function(a){c.loadFileContentAsync(d,b);a.preventDefault()}}else a.setAttribute("class","col bundle-item-type-folder"),f=document.createElement("a"),f.textContent=b.getName(),f.setAttribute("href",""),f.onclick=function(a){c.loadContentAsync(d,b);a.preventDefault()};
...
e,d){this.renderer=e;if(d&&d.items&&Array.isArray(d.items)){var a=this;e=d.name;var b=d.items.filter(function(a){return"title"===a[2]});0<b.length&&(e=markdown.toHTML("#"+b[0][1]).replace(/^<h1>/,"").replace(/<\/h1>$/,""));$(".worksheet-icon").html(e);$(".worksheet-author").html("by "+d.owner);var f=new WorkshhetDirectiveRenderer,k="";d.items.push([null,"eof",null]);d.items.forEach(function(b,d,e){"directive"!=b[2]&&f.applyPendingDirectives(c);if("markup"!=b[2]&&0<k.length){var h=document.createElement("div");
h.innerHTML=markdown.toHTML(k);c.appendChild(h);k=""}switch(b[2]){case "markup":k+=b[1]+"\n\r";break;case "bundle":c.appendChild(a.renderer.render(b[0]));break;case "directive":e.slice(d+1).forEach(function(a){"bundle"==a[2]&&(f.bundleBlock=a[0])}),f.applyDirective(c,b)}});MathJax.Hub.Queue(["Typeset",MathJax.Hub,c.id]);MathJax.Hub.Queue(function(){for(var a=MathJax.Hub.getAllJax(),b=0;b<a.length;b+=1)$(a[b].SourceElement().parentNode).addClass("coda-jax")})}}}();var Competition;
...
($("#fileUploadButton").addClass("disabled"),$("#details").html("Sorry, your browser does not support the HTML5 File API. Please use a newer browser.")):new CodaLab.FileUploader({buttonId:"fileUploadButton",sasEndpoint:"/api/competition/"+a+"/submission/sas",allowedFileTypes:["application/zip","application/x-zip-compressed"],maxFileSizeInBytes:1073741824,validateBeforeFilePrompt:function(){if(0==$("#submission_method_name").length)return!0;var a=$("#submission_method_name").val(),b=$("#submission_method_description").val();
return a&&""!==a&&b&&""!==b},beforeSelection:function(a,b){$("#fileUploadButton").addClass("disabled")},afterSelection:function(a,b){!1===b&&(0<a.files.length&&("type-error"===a.files[0].errors[0].kind?$("#details").html("Please select a valid file. Only ZIP files are accepted."):$("#details").html("The files that you selected is too large. There is a 1GB size limit.")),$("#fileUploadButton").removeClass("disabled"))},uploadProgress:function(a,b,c){b=100*b/c;$("#details").html("Uploading file <em>"+
a.name+"</em>: "+b.toFixed(0)+"% complete.")},uploadError:function(a){$("#details").html("There was an error uploading the file. Please try again.");$("#fileUploadButton").removeClass("disabled")},uploadSuccess:function(b,f){$("#details").html("Creating new submission...");var d=$("#submission_description_textarea").val(),k=$("#submission_method_name").val(),e=$("#submission_method_description").val(),n=$("#submission_project_url").val(),p=$("#submission_publication_url").val(),q=$("#submission_bibtex").val();
$("#submission_description_textarea").val("");$.ajax({url:"/api/competition/"+a+"/submission?description="+encodeURIComponent(d)+"&method_name="+encodeURIComponent(k)+"&method_description="+encodeURIComponent(e)+"&project_url="+encodeURIComponent(n)+"&publication_url="+encodeURIComponent(p)+"&bibtex="+encodeURIComponent(q),type:"post",cache:!1,data:{id:f,name:b.name,type:b.type,size:b.size}}).done(function(a){$("#details").html("");$("#user_results tr.noData").remove();$("#user_results").append(c.displayNewSubmission(a,
d,k,e,n,p,q));$("#user_results #"+a.id+" .glyphicon-plus").on("click",function(){c.showOrHideSubmissionDetails(this)});$("#fileUploadButton").removeClass("disabled");$("#user_results #"+a.id+" .glyphicon-plus").click()}).fail(function(a){var b="An unexpected error occurred.";403==a.status&&(b=a.responseJSON.detail);$("#details").html(b);$("#fileUploadButton").removeClass("disabled")})}});$("#user_results .glyphicon-plus").on("click",function(){c.showOrHideSubmissionDetails(this)})},error:function(a,
b,c){$(".competition_submissions").html("<div class='alert alert-error'>An error occurred. Please try refreshing the page.</div>")}})};c.getPhaseResults=function(a,b){$(".competition_results").html("").append("<div class='competitionPreloader'></div>").children().css({top:"200px",display:"block"});$.ajax({type:"GET",url:"/competitions/"+a+"/results/"+b,cache:!1,success:function(a){$(".competition_results").html("").append(a);$(".column-selectable").click(function(a){a=$(this).closest("table");$(a).find(".column-selected").removeClass();
$(this).addClass("column-selected");var b=$(this).attr("name");a=a.find("td").filter(function(){return $(this).attr("name")===b}).addClass("column-selected");if(0<a.length)for(var c=a.slice().sort(function(a,b){var c=parseInt($.text([$(a).find("span")]));isNaN(c)&&(c=1E5);var f=parseInt($.text([$(b).find("span")]));isNaN(f)&&(f=1E5);return c-f}),f=a[0].parentNode.parentNode,c=c.map(function(){return this.parentNode.cloneNode(!0)}),d=0;d<c.length;d++)$(c[d]).find("td.row-position").text($(c[d]).find("td.column-selected span").text()),
...
cache:!1,success:function(a){console.log(a);"approved"==a.status?location.reload():"pending"==a.status?(b="<div class='participateInfoBlock pendingApproval'>",b+="<div class='infoStatusBar'></div>",b+="<div class='labelArea'>",b+="<label class='regApprovLabel'>Your request to participate in this challenge has been received and a decision is pending.</label>",b+="<label></label>",b+="</div>",b+="</div>"):"denied"==a.status&&(b="<div class='participateInfoBlock'>",b+="<div class='infoStatusBar'></div>",
b+="<div class='labelArea'>",b+="<label class='regDeniedLabel'>Your request to participate in this challenge has been denied. The reason for your denied registrations is: "+a.reason+"</label>",b+="<label></label>",b+="</div>",b+="</div>");location.reload()},error:function(a,b,c){$(".competition_results").html("<div class='alert - error'>An error occurred. Please try refreshing the page.</div>")}});return b};c.displayNewSubmission=function(a,b,f,d,e,m,l){var h=$("#submission_details_template #submission_row_template tr").clone();
$(h).attr("id",a.id.toString());$(h).addClass(c.oddOrEven(a.submission_number));b&&$(h).attr("data-description",$(b).text());f&&$(h).attr("data-method-name",$(f).text());d&&$(h).attr("data-method-description",$(d).text());e&&$(h).attr("data-project-url",$(e).text());m&&$(h).attr("data-publication-url",$(m).text());l&&$(h).attr("data-bibtex",$(l).text());$(h).children().each(function(b){switch(b){case 0:"finished"===a.status&&($(this).val("1"),0<$("#forced_to_leaderboard").length&&($(".glyphicon-ok").remove(),
$($(h).children("td")[4]).html('<span class="glyphicon glyphicon-ok"></span>')));break;case 1:$(this).html(a.submission_number.toString());break;case 2:$(this).html(a.filename);break;case 3:var f=function(a){a=a.toString();1==a.length&&(a="0"+a);return a},d=new Date(a.submitted_at);b=d.getDate().toString()+"/"+d.getMonth().toString()+"/"+d.getFullYear();var e=d.getHours().toString(),k=f(d.getMinutes()),f=f(d.getSeconds());$(this).html(b+" "+e+":"+k+":"+f);break;case 4:$(this).html(c.getSubmissionStatus(a.status))}});
return h};c.oddOrEven=function(a){return a&1?"odd":"even"};c.getSubmissionStatus=function(a){var b="Unknown";"submitting"===a?b="Submitting":"submitted"===a?b="Submitted":"running"===a?b="Running":"failed"===a?b="Failed":"cancelled"===a?b="Cancelled":"finished"===a&&(b="Finished");return b};c.showOrHideSubmissionDetails=function(a){var b=$(a).parents("tr")[0];if($(a).hasClass("glyphicon-minus"))$(a).removeClass("glyphicon-minus"),$(a).addClass("glyphicon-plus"),$(b).next("tr.trDetails").remove();
else{$(a).removeClass("glyphicon-plus");$(a).addClass("glyphicon-minus");a=$("#submission_details_template .trDetails").clone();a.find(".tdDetails").attr("colspan",b.cells.length);a.find("a").each(function(a){$(this).attr("href",$(this).attr("href").replace("_",b.id))});$(b).attr("data-description")&&a.find(".submission_description").html("<b>Description:</b> <br><pre>"+$(b).attr("data-description")+"</pre>");$(b).attr("data-method-name")&&a.find(".submission_method_name").html("<b>Method name:</b> "+
...
'">'+$(b).attr("data-publication-url")+"</a>");$(b).attr("data-bibtex")&&a.find(".submission_bibtex").html("<b>Bibtex:</b><br><pre>"+$(b).attr("data-bibtex")+"</pre>");$(b).attr("data-exception")&&a.find(".traceback").html("Error: <br><pre>"+$(b).attr("data-exception")+"</pre>");var f=$("#phasestate").val(),k=$(b).find("input[name='state']").val();if(1==f&&1==k){var g=a.find("button");g.removeClass("hide");var f=$(b).find(".status").hasClass("submitted"),m=$("#competitionId").val();e(g,f);g.on("click",
