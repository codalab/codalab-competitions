...
return!1};c.showOrHideSubmissionDetails=function(a){var b=$(a).parents("tr")[0];if($(a).hasClass("glyphicon-minus"))$(a).removeClass("glyphicon-minus"),$(a).addClass("glyphicon-plus"),$(b).next("tr.trDetails").remove();else{$(a).removeClass("glyphicon-plus");$(a).addClass("glyphicon-minus");a=$("#submission_details_template .trDetails").clone();a.find(".tdDetails").attr("colspan",b.cells.length);a.find("a").each(function(a){$(this).attr("href",$(this).attr("href").replace("_",b.id))});void 0!==$(b).attr("data-description")&&
...
b+"input:hidden").val("1"),1==$("#phasestate").val()?(0<$("#disallow_leaderboard_modifying").length?($(f).text("Leaderboard modifying disallowed").attr("disabled","disabled"),0<$("#forced_to_leaderboard").length&&($(".glyphicon-ok").remove(),$($("#"+b+" td")[4]).html('<span class="glyphicon glyphicon-ok"></span>'))):0==$("#forced_to_leaderboard").length?($(f).addClass("leaderBoardSubmit"),$(f).text("Submit to Leaderboard")):($(".glyphicon-ok").remove(),$($("#"+b+" td")[4]).html('<span class="glyphicon glyphicon-ok"></span>'),
$(f).removeClass("leaderBoardSubmit"),$(f).addClass("leaderBoardRemove"),$(f).text("Remove from Leaderboard")),$(f).unbind("click").off("click").on("click",function(){d(a,b,$("#cstoken").val(),$(f))})):$(f).addClass("hide");else if("failed"===e.status||"cancelled"===e.status)$(f).addClass("hide"),$(f).parent().parent().find("a").removeClass("hide"),e.exception_details&&$('a[href="traceback/'+b+'/"]').parent().html("Error: <br><pre>"+e.exception_details+"</pre>");$(".competitionPreloader").hide()},
