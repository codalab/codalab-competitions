...
$(f).attr("force_submission_to_leaderboard")?$(f).text("Automatically submitted to leaderboard").attr("disabled","disabled"):($(f).addClass("leaderBoardSubmit"),$(f).text("Submit to Leaderboard"),$(f).on("click",function(){d(a,c,$("#cstoken").val(),$(f))})):$(f).addClass("hide");else if("failed"===e.status||"cancelled"===e.status)$(f).addClass("hide"),$(f).parent().parent().find("a").removeClass("hide");$(".competitionPreloader").hide()},error:function(a,b,c){}})};b.initialize=function(){$(document).ready(function(){$("#participate_form").submit(function(a){a.preventDefault();
