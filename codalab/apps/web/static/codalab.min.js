...
void 0!==l&&""!==l&&$(k).attr("data-team-name",l);void 0!==m&&""!==m&&$(k).attr("data-organization-or-affiliation",m);$(k).children().each(function(b){switch(b){case 0:"finished"===a.status&&($(this).val("1"),0<$("#forced_to_leaderboard").length&&($(".glyphicon-ok").remove(),$($(k).children("td")[4]).html('<span class="glyphicon glyphicon-ok"></span>')));break;case 1:$(this).html(a.submission_number.toString());break;case 2:$(this).html(a.filename);break;case 3:var d=function(a){a=a.toString();1==
a.length&&(a="0"+a);return a},e=new Date(a.submitted_at);b=("0"+(e.getMonth()+1).toString()).slice(-2)+"/"+e.getDate().toString()+"/"+e.getFullYear();var f=("0"+e.getHours().toString()).slice(-2),h=d(e.getMinutes()),d=d(e.getSeconds());$(this).html(b+" "+f+":"+h+":"+d);break;case 4:$(this).html(c.getSubmissionStatus(a.status))}});return k};c.oddOrEven=function(a){return a&1?"odd":"even"};c.getSubmissionStatus=function(a){var b="Unknown";"submitting"===a?b="Submitting":"submitted"===a?b="Submitted":
"running"===a?b="Running":"failed"===a?b="Failed":"cancelled"===a?b="Cancelled":"finished"===a&&(b="Finished");return b};c.toggleSubmissionPublic=function(a){a.preventDefault();this.blur();a=$(this).attr("submission-id");var b=$(this);$.get("/my/competition/submission/"+a+"/toggle_make_public").success(function(a){b.html("Make your submission "+("True"==a?"private":"public"))}).error(function(){alert("Error making submission public, is your Internet connection working?")});return!1};c.updateSubmissionDescription=
function(a){a.preventDefault();var b=this;b.blur();a=$(this).attr("submission-id");var d=$(this).parent().find('textarea[name="updated_description"]').val()||"",d=d.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");$.post("/competitions/update_description/"+a,{updated_description:d}).success(function(){$(b).parent().find(".submission_description").html("<b>Description:</b> <br><pre>"+d+"</pre>");$(b).parent().find('textarea[name="updated_description"]').val("").hide();$(b).parent().find(".update_description_btn").removeClass("hide").show();
$(b).hide()}).error(function(){alert("Error updating description, is your Internet connection working?")})};c.showOrHideSubmissionDetails=function(a){var b=$(a).parents("tr")[0];if($(a).hasClass("glyphicon-minus"))$(a).removeClass("glyphicon-minus"),$(a).addClass("glyphicon-plus"),$(b).next("tr.trDetails").remove();else{$(a).removeClass("glyphicon-plus");$(a).addClass("glyphicon-minus");a=$("#submission_details_template .trDetails").clone();a.find(".tdDetails").attr("colspan",b.cells.length);a.find("a").each(function(a){$(this).attr("href",
$(this).attr("href").replace("_",b.id))});void 0!==$(b).attr("data-description")&&a.find(".submission_description").html("<b>Description:</b> <br><pre>"+$(b).attr("data-description")+"</pre>");$(b).attr("data-team-name")&&a.find(".submission_team_name").html("<b>Team name:</b> "+$(b).attr("data-team-name"));$(b).attr("data-method-name")&&a.find(".submission_method_name").html("<b>Method name:</b> "+$(b).attr("data-method-name"));$(b).attr("data-method-description")&&a.find(".submission_method_description").html("<b>Method description:</b><br><p>"+
$(b).attr("data-method-description")+"</p>");$(b).attr("data-project-url")&&a.find(".submission_project_url").html('<b>Project URL:</b> <a href="'+$(b).attr("data-project-url")+'">'+$(b).attr("data-project-url")+"</a>");$(b).attr("data-publication-url")&&a.find(".submission_publication_url").html('<b>Publication URL:</b> <a href="'+$(b).attr("data-publication-url")+'">'+$(b).attr("data-publication-url")+"</a>");$(b).attr("data-bibtex")&&a.find(".submission_bibtex").html("<b>Bibtex:</b><br><pre>"+
$(b).attr("data-bibtex")+"</pre>");$(b).attr("data-organization-or-affiliation")&&a.find(".submission_organization_or_affiliation").html("<b>Organization/affiliation:</b> "+$(b).attr("data-organization-or-affiliation"));$(b).attr("data-exception")&&a.find(".traceback").html("Error: <br><pre>"+$(b).attr("data-exception")+"</pre>");var d=$(b).attr("data-is-public")?"private":"public";a.find(".public_link").html("Make your submission "+d);a.find(".public_link").click(c.toggleSubmissionPublic);a.find(".public_link").attr("submission-id",
...
e=$(b).find("input[name='state']").val();if(1==d&&1==e){var f=a.find(".leaderboard_button");f.removeClass("hide");var d=$(b).find(".status").hasClass("submitted"),k=$("#competitionId").val();g(f,d);f.on("click",function(){h(k,b.id,$("#cstoken").val(),f)})}else{d=$.trim($(b).find(".statusName").html());f=a.find(".leaderboard_button").addClass("hide");if("Submitting"===d||"Submitted"===d||"Running"===d)f.removeClass("hide"),f.text("Refresh status"),f.on("click",function(){c.updateSubmissionStatus($("#competitionId").val(),
...
b+"input:hidden").val("1"),1==$("#phasestate").val()?(0<$("#disallow_leaderboard_modifying").length?($(d).text("Leaderboard modifying disallowed").attr("disabled","disabled"),0<$("#forced_to_leaderboard").length&&($(".glyphicon-ok").remove(),$($("#"+b+" td")[4]).html('<span class="glyphicon glyphicon-ok"></span>'))):0==$("#forced_to_leaderboard").length?($(d).addClass("leaderBoardSubmit"),$(d).text("Submit to Leaderboard")):($(".glyphicon-ok").remove(),$($("#"+b+" td")[4]).html('<span class="glyphicon glyphicon-ok"></span>'),
$(d).removeClass("leaderBoardSubmit"),$(d).addClass("leaderBoardRemove"),$(d).text("Remove from Leaderboard")),$(d).unbind("click").off("click").on("click",function(){h(a,b,$("#cstoken").val(),$(d))})):$(d).addClass("hide");else if("failed"===e.status||"cancelled"===e.status)$(d).addClass("hide"),$(d).parent().parent().find("a").removeClass("hide"),e.exception_details&&$('a[href="traceback/'+b+'/"]').parent().html("Error: <br><pre>"+e.exception_details+"</pre>");$(".competitionPreloader").hide()},
error:function(a,b,d){}})};c.initialize=function(){$(document).ready(function(){$("#participate_form").submit(function(a){a.preventDefault();if($("#participateButton").hasClass("disabledStatus"))return!1;$("#result").html("");a=$(this).serialize();var b=$("#competitionId").val();request=$.ajax({url:"/api/competition/"+b+"/participate/",type:"post",dataType:"text",data:a,success:function(a,e,f){$(".content form").replaceWith(c.displayRegistrationStatus(b))},error:function(a,b,c){alert("There was a problem registering for this competition.")}});
return!1});$("#submissions_phase_buttons .btn").each(function(a,b){$(this).click(function(){var a=$.trim($(this).attr("id").replace("submissions_phase_",""));$("#submissions_phase_buttons .btn").removeClass("selected");$(this).addClass("selected");var b=$("#competitionId").val(),f=$("#cstoken").val();c.getPhaseSubmissions(b,a,f)})});$("#results_phase_buttons .btn").each(function(a,b){$(this).click(function(){var a=$.trim($(this).attr("id").replace("results_phase_",""));$("#results_phase_buttons .btn").removeClass("selected");
$(this).addClass("selected");var b=$("#competitionId").val();c.getPhaseResults(b,a)})});$("#results_phase_submissions .btn").click(function(a){a=$.trim($(this).attr("id").replace("public_results_phase_",""));var b=$("#competitionId").val();c.getPublicPhaseSubmisisons(b,a)});$(".top-bar-section ul > li").removeClass("active");$("#liCompetitions").addClass("active");$("#competition-publish-button").click(function(a){a.preventDefault();var b=$(this).parent()[0];request=$.ajax({url:$(this).attr("href"),
success:function(a,c,f){console.log("Published competition.");$(b).children("#competition-publish-button").hide();$(b).children("#competition-delete-button").hide();$(b).children("#competition-unpublish-button").show()},error:function(a,b,c){a=$.parseJSON(a.responseJSON);a.error&&alert(a.error);console.log("Error publishing competition!")}})});$("#competition-unpublish-button").click(function(a){a.preventDefault();var b=$(this).parent()[0];request=$.ajax({url:$(this).attr("href"),success:function(a,
c,f){console.log("Unpublished competition.");$(b).children("#competition-publish-button").show();$(b).children("#competition-delete-button").show();$(b).children("#competition-unpublish-button").hide()},error:function(a,b,c){console.log("Error unpublishing competition!")}})});$("#my_managing .competition-tile .competition-actions").each(function(a,b){"True"==$(this)[0].getAttribute("published")?($(this).find("#competition-delete-button").hide(),$(this).find("#competition-publish-button").hide(),$(this).find("#competition-unpublish-button").show()):
