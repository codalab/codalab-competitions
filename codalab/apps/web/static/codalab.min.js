...
"",q=$("#submission_publication_url").val()||"",r=$("#submission_bibtex").val()||"",t=$("#submission_team_name").val()||"",u=$("#submission_organization_or_affiliation").val()||"",v=$("#submission_phase_id").val();$("#submission_description_textarea").val("");$.ajax({url:"/api/competition/"+a+"/submission?description="+encodeURIComponent(d)+"&method_name="+encodeURIComponent(h)+"&method_description="+encodeURIComponent(e)+"&project_url="+encodeURIComponent(k)+"&publication_url="+encodeURIComponent(q)+
"&team_name="+encodeURIComponent(t)+"&organization_or_affiliation="+encodeURIComponent(u)+"&bibtex="+encodeURIComponent(r)+"&phase_id="+encodeURIComponent(v),type:"post",cache:!1,data:{id:f,name:b.name,type:b.type,size:b.size}}).done(function(a){$("#details").html("");$("#user_results tr.noData").remove();$("#user_results").append(c.displayNewSubmission(a,d,h,e,k,q,r,t,u));$("#user_results #"+a.id+" .glyphicon-plus").on("click",function(){c.showOrHideSubmissionDetails(this)});$("#fileUploadButton").removeClass("disabled");
$("#user_results #"+a.id+" .glyphicon-plus").click()}).fail(function(a){var b="An unexpected error occurred.";403==a.status&&(b=a.responseJSON.detail);$("#details").html(b);$("#fileUploadButton").removeClass("disabled")})}});$("#user_results .glyphicon-plus").on("click",function(){c.showOrHideSubmissionDetails(this)})},error:function(a,b,c){$(".competition_submissions").html("<div class='alert alert-error'>An error occurred. Please try refreshing the page.</div>")}})};c.getPhaseResults=function(a,
b){$(".competition_results").html("").append("<div class='competitionPreloader'></div>").children().css({top:"200px",display:"block"});$.ajax({type:"GET",url:"/competitions/"+a+"/results/"+b,cache:!1,success:function(a){$(".competition_results").html("").append(a);$(".column-selectable").click(function(a){a=$(this).closest("table");$(a).find(".column-selected").removeClass();$(this).addClass("column-selected");var b=$(this).attr("name");a=a.find("td").filter(function(){return $(this).attr("name")===
...
a.reason+"</label>",b+="<label></label>",b+="</div>",b+="</div>");location.reload()},error:function(a,b,c){$(".competition_results").html("<div class='alert - error'>An error occurred. Please try refreshing the page.</div>")}});return b};c.displayNewSubmission=function(a,b,f,d,e,n,l,m,p){var k=$("#submission_details_template #submission_row_template tr").clone();$(k).attr("id",a.id.toString());$(k).addClass(c.oddOrEven(a.submission_number));void 0!==b&&""!==b&&$(k).attr("data-description",b);void 0!==
f&&""!==f&&$(k).attr("data-method-name",f);void 0!==d&&""!==d&&$(k).attr("data-method-description",d);void 0!==e&&""!==e&&$(k).attr("data-project-url",e);void 0!==n&&""!==n&&$(k).attr("data-publication-url",n);void 0!==l&&""!==l&&$(k).attr("data-bibtex",l);void 0!==m&&""!==m&&$(k).attr("data-team-name",m);void 0!==p&&""!==p&&$(k).attr("data-organization-or-affiliation",p);$(k).children().each(function(b){switch(b){case 0:"finished"===a.status&&($(this).val("1"),0<$("#forced_to_leaderboard").length&&
($(".glyphicon-ok").remove(),$($(k).children("td")[4]).html('<span class="glyphicon glyphicon-ok"></span>')));break;case 1:$(this).html(a.submission_number.toString());break;case 2:$(this).html(a.filename);break;case 3:var f=function(a){a=a.toString();1==a.length&&(a="0"+a);return a},d=new Date(a.submitted_at);b=("0"+(d.getMonth()+1).toString()).slice(-2)+"/"+d.getDate().toString()+"/"+d.getFullYear();var e=("0"+d.getHours().toString()).slice(-2),h=f(d.getMinutes()),f=f(d.getSeconds());$(this).html(b+
" "+e+":"+h+":"+f);break;case 4:$(this).html(c.getSubmissionStatus(a.status))}});return k};c.oddOrEven=function(a){return a&1?"odd":"even"};c.getSubmissionStatus=function(a){var b="Unknown";"submitting"===a?b="Submitting":"submitted"===a?b="Submitted":"running"===a?b="Running":"failed"===a?b="Failed":"cancelled"===a?b="Cancelled":"finished"===a&&(b="Finished");return b};c.toggleSubmissionPublic=function(a){a.preventDefault();this.blur();a=$(this).attr("submission-id");var b=$(this);$.get("/my/competition/submission/"+
a+"/toggle_make_public").success(function(a){b.html("Make your submission "+("True"==a?"private":"public"))}).error(function(){alert("Error making submission public, is your Internet connection working?")});return!1};c.updateSubmissionDescription=function(a){a.preventDefault();var b=this;b.blur();a=$(this).attr("submission-id");var c=$(this).parent().find('textarea[name="updated_description"]').val()||"",c=c.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");$.post("/competitions/update_description/"+
a,{updated_description:c}).success(function(){$(b).parent().find(".submission_description").html("<b>Description:</b> <br><pre>"+c+"</pre>");$(b).parent().find('textarea[name="updated_description"]').val("").hide();$(b).parent().find(".update_description_btn").removeClass("hide").show();$(b).hide()}).error(function(){alert("Error updating description, is your Internet connection working?")})};c.showOrHideSubmissionDetails=function(a){var b=$(a).parents("tr")[0];if($(a).hasClass("glyphicon-minus"))$(a).removeClass("glyphicon-minus"),
$(a).addClass("glyphicon-plus"),$(b).next("tr.trDetails").remove();else{$(a).removeClass("glyphicon-plus");$(a).addClass("glyphicon-minus");a=$("#submission_details_template .trDetails").clone();a.find(".tdDetails").attr("colspan",b.cells.length);a.find("a").each(function(a){$(this).attr("href",$(this).attr("href").replace("_",b.id))});void 0!==$(b).attr("data-description")&&a.find(".submission_description").html("<b>Description:</b> <br><pre>"+$(b).attr("data-description")+"</pre>");$(b).attr("data-team-name")&&
a.find(".submission_team_name").html("<b>Team name:</b> "+$(b).attr("data-team-name"));$(b).attr("data-method-name")&&a.find(".submission_method_name").html("<b>Method name:</b> "+$(b).attr("data-method-name"));$(b).attr("data-method-description")&&a.find(".submission_method_description").html("<b>Method description:</b><br><pre>"+$(b).attr("data-method-description")+"</pre>");$(b).attr("data-project-url")&&a.find(".submission_project_url").html('<b>Project URL:</b> <a href="'+$(b).attr("data-project-url")+
'">'+$(b).attr("data-project-url")+"</a>");$(b).attr("data-publication-url")&&a.find(".submission_publication_url").html('<b>Publication URL:</b> <a href="'+$(b).attr("data-publication-url")+'">'+$(b).attr("data-publication-url")+"</a>");$(b).attr("data-bibtex")&&a.find(".submission_bibtex").html("<b>Bibtex:</b><br><pre>"+$(b).attr("data-bibtex")+"</pre>");$(b).attr("data-organization-or-affiliation")&&a.find(".submission_organization_or_affiliation").html("<b>Organization/affiliation:</b> "+$(b).attr("data-organization-or-affiliation"));
$(b).attr("data-exception")&&a.find(".traceback").html("Error: <br><pre>"+$(b).attr("data-exception")+"</pre>");var f=$(b).attr("data-is-public")?"private":"public";a.find(".public_link").html("Make your submission "+f);a.find(".public_link").click(c.toggleSubmissionPublic);a.find(".public_link").attr("submission-id",b.id);a.find(".save_description_btn").click(c.updateSubmissionDescription);a.find(".save_description_btn").attr("submission-id",b.id);a.find(".update_description_btn").click(function(){var a=
$(this).parent().find(".submission_description").text(),a=-1!==a.indexOf("Description: ")?a.substr(13):"";$(this).parent().find(".save_description_btn").removeClass("hide").show();$(this).parent().find('textarea[name="updated_description"]').removeClass("hide").show().val(a);$(this).hide()});var f=$("#phasestate").val(),h=$(b).find("input[name='state']").val();if(1==f&&1==h){var g=a.find(".leaderboard_button");g.removeClass("hide");var f=$(b).find(".status").hasClass("submitted"),n=$("#competitionId").val();
e(g,f);g.on("click",function(){d(n,b.id,$("#cstoken").val(),g)})}else{f=$.trim($(b).find(".statusName").html());g=a.find(".leaderboard_button").addClass("hide");if("Submitting"===f||"Submitted"===f||"Running"===f)g.removeClass("hide"),g.text("Refresh status"),g.on("click",function(){c.updateSubmissionStatus($("#competitionId").val(),b.id,this)});"Failed"!==f&&"Cancelled"!==f||a.find("a").removeClass("hide")}$(b).after(a)}};c.updateSubmissionStatus=function(a,b,f){$(f).parents(".submission_details").find(".preloader-handler").append("<div class='competitionPreloader'></div>").children().css({top:"25px",
display:"block"});$.ajax({type:"GET",url:"/api/competition/"+a+"/submission/"+b,cache:!1,success:function(e){$("#user_results #"+b).find(".statusName").html(c.getSubmissionStatus(e.status));if("finished"===e.status)$("#user_results #"+b+"input:hidden").val("1"),1==$("#phasestate").val()?(0<$("#disallow_leaderboard_modifying").length?($(f).text("Leaderboard modifying disallowed").attr("disabled","disabled"),0<$("#forced_to_leaderboard").length&&($(".glyphicon-ok").remove(),$($("#"+b+" td")[4]).html('<span class="glyphicon glyphicon-ok"></span>'))):
0==$("#forced_to_leaderboard").length?($(f).addClass("leaderBoardSubmit"),$(f).text("Submit to Leaderboard")):($(".glyphicon-ok").remove(),$($("#"+b+" td")[4]).html('<span class="glyphicon glyphicon-ok"></span>'),$(f).removeClass("leaderBoardSubmit"),$(f).addClass("leaderBoardRemove"),$(f).text("Remove from Leaderboard")),$(f).unbind("click").off("click").on("click",function(){d(a,b,$("#cstoken").val(),$(f))})):$(f).addClass("hide");else if("failed"===e.status||"cancelled"===e.status)$(f).addClass("hide"),
$(f).parent().parent().find("a").removeClass("hide"),e.exception_details&&$('a[href="traceback/'+b+'/"]').parent().html("Error: <br><pre>"+e.exception_details+"</pre>");$(".competitionPreloader").hide()},error:function(a,b,c){}})};c.initialize=function(){$(document).ready(function(){$("#participate_form").submit(function(a){a.preventDefault();if($("#participateButton").hasClass("disabledStatus"))return!1;$("#result").html("");a=$(this).serialize();var b=$("#competitionId").val();request=$.ajax({url:"/api/competition/"+
...
d=$("#cstoken").val();c.getPhaseSubmissions(b,a,d)})});$("#results_phase_buttons .btn").each(function(a,b){$(this).click(function(){var a=$.trim($(this).attr("id").replace("results_phase_",""));$("#results_phase_buttons .btn").removeClass("selected");$(this).addClass("selected");var b=$("#competitionId").val();c.getPhaseResults(b,a)})});$(".top-bar-section ul > li").removeClass("active");$("#liCompetitions").addClass("active");$("#competition-publish-button").click(function(a){a.preventDefault();
...
request=$.ajax({url:$(this).attr("href"),success:function(a,c,d){console.log("Unpublished competition.");$(b).children("#competition-publish-button").show();$(b).children("#competition-delete-button").show();$(b).children("#competition-unpublish-button").hide()},error:function(a,b,c){console.log("Error unpublishing competition!")}})});$("#my_managing .competition-tile .competition-actions").each(function(a,b){"True"==$(this)[0].getAttribute("published")?($(this).find("#competition-delete-button").hide(),
$(this).find("#competition-publish-button").hide(),$(this).find("#competition-unpublish-button").show()):($(this).find("#competition-delete-button").show(),$(this).find("#competition-publish-button").show(),$(this).find("#competition-unpublish-button").hide())})})}})(Competition||(Competition={}));var csrftoken;function csrfSafeMethod(c){return/^(GET|HEAD|OPTIONS|TRACE)$/.test(c)}function sameOrigin(c){var e="//"+document.location.host,d=document.location.protocol+e;return c==d||c.slice(0,d.length+1)==d+"/"||c==e||c.slice(0,e.length+1)==e+"/"||!/^(\/\/|http:|https:).*/.test(c)}$(function(){csrftoken=$.cookie("csrftoken");$.ajaxSetup({beforeSend:function(c,e){!csrfSafeMethod(e.type)&&sameOrigin(e.url)&&c.setRequestHeader("X-CSRFToken",csrftoken)}})});var CodaLab;
