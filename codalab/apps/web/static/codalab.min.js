...
b,c,null)}),d=d.concat(b).sort(function(b,a){return b.getName().localeCompare(a.getName())}));c.setChildren(d);a.renderTable(e,a.getContentTableModel(c,e))},error:function(b,a,c){}})};a.loadFileContentAsync=function(e,c){$.ajax({type:"GET",url:[c.getUrl().replace("api/bundles/content","api/bundles/filecontent"),c.getFullName()].join("/"),cache:!1,success:function(b){c.setData(b);a.renderTable(e,a.getContentTableModel(c,e))},error:function(b,a,c){}})};a.getContentTableModel=function(e,c){var b=0;!1==
...
return c.get(0)};return a}(),WorkshhetDirectiveRenderer=function(){return function(){var a=this;a.display="inline";a.bundleBlock=null;a.tableDirective=[];a.applyDirective=function(e,c){switch(c[1].directive){case "display":a.display=c[1].display;a.applyPendingDirectives(e);break;case "image":$(e).append($("<img />",{src:"/api/bundles/filecontent/"+a.bundleBlock.uuid+"/"+encodeURI(c[1].path),"class":"bundleImage",alt:c[1].name}));break;case "metadata":a.tableDirective.push(c[1])}};a.insertRowTable=
function(a,c,b){var d=$("<tr></tr>");c.forEach(function(a){d.append($(b).text(a))});a.append(d)};a.getDataFromPath=function(e){return a.bundleBlock.metadata[e]};a.applyPendingDirectives=function(e){if(0<a.tableDirective.length){var c=$("<table></table>");if("table"===a.display){var b=$("<thead></thead>");a.insertRowTable(b,a.tableDirective.map(function(b){return b.name}),"<th></th>")}var d=$("<tbody></tbody>");"table"===a.display?a.insertRowTable(d,a.tableDirective.map(function(b){return a.getDataFromPath(b.path)}),
"<td></td>"):a.tableDirective.forEach(function(b){a.insertRowTable(d,[b.name,a.getDataFromPath(b.path)],"<td></td>")});c.append(b);c.append(d);$(e).append(c);a.tableDirective=[]}}}}(),WorksheetRenderer=function(){return function(a,e,c){this.renderer=e;if(c&&c.items&&Array.isArray(c.items)){var b=this;e=c.name;if(0<c.items.length){var d=c.items[0];null===d[0]&&0==d[1].indexOf("#")&&(e=markdown.toHTML(d[1]).replace(/^<h1>/,"").replace(/<\/h1>$/,""),c.items=c.items.slice(1))}$(".worksheet-icon").html(e);
$(".worksheet-author").html("by codalab");var f=new WorkshhetDirectiveRenderer,h="";c.items.push([null,"eof",null]);c.items.forEach(function(d,c,e){"directive"!=d[2]&&f.applyPendingDirectives(a);if("markup"!=d[2]&&0<h.length){var m=document.createElement("div");m.innerHTML=markdown.toHTML(h);a.appendChild(m);h=""}switch(d[2]){case "markup":h+=d[1]+"\n\r";break;case "bundle":d[1]&&a.appendChild(b.renderer.render(d[0]));break;case "directive":e.slice(c+1).forEach(function(b){"bundle"==b[2]&&(f.bundleBlock=
b[0])}),f.applyDirective(a,d)}});MathJax.Hub.Queue(["Typeset",MathJax.Hub,a.id]);MathJax.Hub.Queue(function(){for(var b=MathJax.Hub.getAllJax(),a=0;a<b.length;a+=1)$(b[a].SourceElement().parentNode).addClass("coda-jax")})}}}();var Competition;
(function(a){function e(b,a){b.attr("force_submission_to_leaderboard")?a&&b.text("Automatically submitted to leaderboard").attr("disabled","disabled"):a?(b.removeClass("leaderBoardSubmit"),b.addClass("leaderBoardRemove"),b.text("Remove from Leaderboard")):(b.removeClass("leaderBoardRemove"),b.addClass("leaderBoardSubmit"),b.text("Submit to Leaderboard"))}function c(b,a,c,h){b="/api/competition/"+b+"/submission/"+a+"/leaderboard";var g="delete";h.hasClass("leaderBoardSubmit")&&(g="post");request=$.ajax({url:b,
type:g,datatype:"text",data:{csrfmiddlewaretoken:c},success:function(b,c,f){(b="post"==g)?(c=$("#user_results td.status"),c.removeClass("submitted"),c.addClass("not_submitted"),c.html(""),c=$("#"+a+" td.status"),c.addClass("submitted"),c.html("<i class='fi-check'></i>"),$("#user_results button.leaderBoardRemove").each(function(b){e($(this),!1)})):(c=$("#"+a+" td.status"),c.removeClass("submitted"),c.addClass("not_submitted"),c.html(""));e(h,b)},error:function(b,a,d){alert("An error occurred. Please try again or report the issue")},
beforeSend:function(b){b.setRequestHeader("X-CSRFToken",c)}})}a.invokePhaseButtonOnOpen=function(b){var a=$("#"+b+" .button.selected")[0];void 0===a&&(a=$("#"+b+" .button.active")[0],void 0===a&&(a=$("#"+b+" .button")[0]));a.click()};a.getPhaseSubmissions=function(b,d,c){$(".competition_submissions").html("").append("<div class='competitionPreloader'></div>").children().css({top:"200px",display:"block"});$.ajax({type:"GET",url:"/competitions/"+b+"/submissions/"+d,cache:!1,success:function(d){$(".competition_submissions").html("").append(d);
...
a&&(0<b.files.length&&("type-error"===b.files[0].errors[0].kind?$("#details").html("Please select a valid file. Only ZIP files are accepted."):$("#details").html("The files that you selected is too large. There is a 1GB size limit.")),$("#fileUploadButton").removeClass("disabled"))},uploadProgress:function(b,a,d){a=100*a/d;$("#details").html("Uploading file <em>"+b.name+"</em>: "+a.toFixed(0)+"% complete.")},uploadError:function(b){$("#details").html("There was an error uploading the file. Please try again.");
...
e).toString();var l=e.getHours().toString(),k=c(e.getMinutes()),c=c(e.getSeconds());$(this).html(d+" "+l+":"+k+":"+c);break;case 4:$(this).html(a.getSubmissionStatus(b.status))}});return d};a.oddOrEven=function(b){return b&1?"odd":"even"};a.getSubmissionStatus=function(b){var a="Unknown";"submitting"===b?a="Submitting":"submitted"===b?a="Submitted":"running"===b?a="Running":"failed"===b?a="Failed":"cancelled"===b?a="Cancelled":"finished"===b&&(a="Finished");return a};a.showOrHideSubmissionDetails=
...
b.find("button");g.removeClass("hide");var f=$(d).find(".status").hasClass("submitted"),l=$("#competitionId").val();e(g,f);g.on("click",function(){c(l,d.id,$("#cstoken").val(),g)})}else{f=$.trim($(d).find(".statusName").html());g=b.find("button").addClass("hide");if("Submitting"===f||"Submitted"===f||"Running"===f)g.removeClass("hide"),g.text("Refresh status"),g.on("click",function(){a.updateSubmissionStatus($("#competitionId").val(),d.id,this)});"Failed"!==f&&"Cancelled"!==f||b.find("a").removeClass("hide")}$(d).after(b)}};
...
(function(a){var e=function(){function a(b){var d=this;this.defaultOptions={buttonId:"fileUploadButton",disabledClassName:"disabled",beforeSelection:function(){},afterSelection:function(b,a){},uploadProgress:function(b,a,d){},uploadSuccess:function(b){},uploadError:function(b){},allowedFileTypes:void 0,extensionToFileType:{zip:"application/zip"},maxFileSizeInBytes:void 0,maxBlockSizeInBytes:1048576};this.options=$.extend({},this.defaultOptions,b);var c=$("#"+this.options.buttonId);this.setupFileInput();
c.after(this.fileInput);c.on("click",function(b){c.hasClass(d.options.disabledClassName)||d.fileInput.click()})}a.prototype.setupFileInput=function(){var b=this,d=this.fileInput;this.fileInput=$(document.createElement("input"));this.fileInput.attr("type","file");this.fileInput.attr("style","visibility: hidden; width: 1px; height: 1px;");this.fileInput.on("change",function(d){b.options.beforeSelection();d=b.validate();var e=0<d.files.length&&!0===a.selectionIsValid(d);b.options.afterSelection(d,e);
e?b.beginUpload(d,0):b.setupFileInput()});void 0!==d&&d.replaceWith(this.fileInput)};a.prototype.validate=function(){var b=this,a=this.fileInput.get(0).files,c=[];0<a.length&&$.each(a,function(a,d){var e=[];b.options.maxFileSizeInBytes&&d.size>b.options.maxFileSizeInBytes&&e.push({kind:"size-error"});var k=d.type;if(""===k){var m=d.name.split(".");1<m.length&&(k=b.options.extensionToFileType[m.pop().toLowerCase()])}b.options.allowedFileTypes&&-1===$.inArray(k,b.options.allowedFileTypes)&&e.push({kind:"type-error"});
c.push({file:d,errors:e})});return{files:c}};a.selectionIsValid=function(b){return 0===b.files.map(function(b){return b.errors.length}).reduce(function(b,a,c,e){return b+a},0)};a.prototype.getCurrentFile=function(){if(void 0!==this.state)return this.state.selection.files[this.state.fileIndex].file};a.prototype.getFileReader=function(){var b=this,a=new FileReader;a.onload=function(a){var d=b.state.sasUrl+"&comp=block&blockid="+b.state.blockIds[b.state.blockIds.length-1];a=new Uint8Array(a.target.result);
var c=b.state.sasVersion;$.ajax({url:d,type:"PUT",data:a,processData:!1,beforeSend:function(b){b.setRequestHeader("x-ms-version",c);b.setRequestHeader("x-ms-blob-type","BlockBlob")},success:function(a,d){var c=b.getCurrentFile();b.options.uploadProgress(c,b.state.bytesUploaded,c.size);b.state.bytesUploaded<c.size?b.upload():b.endUpload()},error:function(a,d,c){b.options.uploadError({kind:"write-error",jqXHR:a,file:b.getCurrentFile()})}})};a.onerror=function(a){b.options.uploadError({kind:"read-error",
message:a.message,file:b.getCurrentFile()})};return a};a.prototype.beginUpload=function(b,a){this.state={sasUrl:void 0,sasVersion:void 0,trackingId:void 0,reader:void 0,selection:b,fileIndex:a,blockIds:[],bytesUploaded:0};var c=this.getCurrentFile(),e=this;$.ajax({url:e.options.sasEndpoint,type:"POST",data:{name:c.name,type:c.type,size:c.size},success:function(a,b){e.state.sasUrl=a.url;e.state.sasVersion=a.version;e.state.trackingId=a.id;e.state.reader=e.getFileReader();e.options.uploadProgress(c,
e.state.bytesUploaded,c.size);e.upload()},error:function(a,b,c){e.options.uploadError({kind:"sas-error",jqXHR:a,file:e.getCurrentFile()})}})};a.prototype.upload=function(){var a=this.getCurrentFile(),c=this.state.bytesUploaded,e=c+Math.min(a.size-c,this.options.maxBlockSizeInBytes),a=a.slice(c,e),c="000000"+(this.state.blockIds.length+1),c=btoa(c.substring(c.length-6));this.state.blockIds.push(c);this.state.bytesUploaded=e;this.state.reader.readAsArrayBuffer(a)};a.prototype.endUpload=function(){var a=
this.state.sasUrl+"&comp=blocklist",c=this.getCurrentFile(),e=['<?xml version="1.0" encoding="utf-8"?>'];e.push("<BlockList>");for(var h=0;h<this.state.blockIds.length;h++)e.push("  <Latest>"+this.state.blockIds[h]+"</Latest>");e.push("</BlockList>");var g=this;$.ajax({url:a,type:"PUT",contentType:"application/xml",processData:!1,data:e.join("\n"),beforeSend:function(a){a.setRequestHeader("x-ms-version",g.state.sasVersion);a.setRequestHeader("x-ms-blob-content-type",c.type);a.setRequestHeader("x-ms-meta-name",
c.name);a.setRequestHeader("x-ms-meta-size",c.size.toString())},success:function(a,b){g.options.uploadSuccess(c,g.state.trackingId);var e=1+g.state.fileIndex;e<g.state.selection.files.length?g.beginUpload(g.state.selection,e):g.setupFileInput()},error:function(a,b,c){g.options.uploadError({kind:"write-error",jqXHR:a,file:g.getCurrentFile()})}})};a.isSupported=function(){return window.File&&window.FileReader&&window.FileList&&window.Blob?!0:!1};return a}();a.FileUploader=e;(function(a){a.CreateReady=
function(){!1===e.isSupported()?($("#uploadButton").addClass("disabled"),$("#details").html("Sorry, your browser does not support the HTML5 File API. Please use a newer browser.")):new e({buttonId:"uploadButton",sasEndpoint:"/api/competition/create/sas",allowedFileTypes:["application/zip","application/x-zip-compressed"],maxFileSizeInBytes:1073741824,beforeSelection:function(a,c){$("#uploadButton").addClass("disabled")},afterSelection:function(a,c){!1===c&&(0<a.files.length&&("type-error"===a.files[0].errors[0].kind?
$("#details").html("Please select a valid file. Only ZIP files are accepted."):$("#details").html("The files that you selected is too large. There is a 1GB size limit.")),$("#uploadButton").removeClass("disabled"))},uploadProgress:function(a,c,e){c=100*c/e;$("#details").html("Uploading file <em>"+a.name+"</em>: "+c.toFixed(0)+"% complete.")},uploadError:function(a){$("#details").html("There was an error uploading the file. Please try again.");$("#uploadButton").removeClass("disabled")},uploadSuccess:function(a,
