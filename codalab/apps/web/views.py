    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            if self.request.user.is_authenticated() and self.request.user in [x.user for x in competition.participants.all()]:
                context['my_status'] = [
                    x.status for x in competition.participants.all() if x.user == self.request.user][0].codename
                ...
                for phase in competition.phases.all():
                    submissions[phase] = models.CompetitionSubmission.objects.filter(
                        participant=context['my_participant'], phase=phase)
                    if phase.is_active:
...
class CompetitionSubmissionsPage(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(CompetitionSubmissionsPage,
        ...
        if self.request.user in [x.user for x in competition.participants.all()]:
            ...
            if participant.status.codename == models.ParticipantStatus.APPROVED:
                ...
                submissions = models.CompetitionSubmission.objects.filter(
                    participant=participant, phase=phase)
                context['my_submissions'] = submissions
                context['phase'] = phase
                ids = [e.result.id for e in models.PhaseLeaderBoardEntry.objects.filter(
                    board__phase=phase) if e.result in submissions]
                context['id_of_submission_in_leaderboard'] = ids[
                    0] if len(ids) > 0 else -1
        return context
...
class CompetitionResultsPage(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(CompetitionResultsPage,
                        self).get_context_data(**kwargs)
        competition = models.Competition.objects.get(pk=self.kwargs['id'])
        phase = competition.phases.get(pk=self.kwargs['phase'])
        context['phase'] = phase
...
class CompetitionResultsDownload(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response = HttpResponse(
            csvfile.getvalue(), status=200, content_type="text/csv")
...
class MyCompetitionParticipantView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super(MyCompetitionParticipantView,
                    self).get_context_data(**kwargs)
...
class CompetitionIndexPartial(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super(CompetitionIndexPartial,
                        self).get_context_data(**kwargs)
...
class MySubmissionResultsPartial(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super(MySubmissionResultsPartial,
                    self).get_context_data(**kwargs)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if hasattr(submission, fileattr):
            ...
            if f:
                try:
                    resp = HttpResponse(
                        f.read(), status=200, content_type='text/plain' if ext == 'txt' else 'application/zip')
                    ...
                    resp = HttpResponse(
                        "There was an error retrieving file '%s'. Please try again later or report the issue." %
                        filetype, status=200, content_type='text/plain')
