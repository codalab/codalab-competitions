...
def competition_index(request):
    ...
    competitions = models.Competition.objects.filter(
        published=True).order_by('-end_date')

    if query:
        competitions = competitions.filter(
            Q(title__iregex=".*%s" % query) | Q(description__iregex=".*%s" % query))
...
def my_index(request):
    ...
    try:
        denied = models.ParticipantStatus.objects.get(
            codename=models.ParticipantStatus.DENIED)
    except:
        denied = -1

    competitions_owner = models.Competition.objects.filter(
        creator=request.user)
    competitions_admin = models.Competition.objects.filter(
        admins__in=[
            request.user])
    published_competitions = models.Competition.objects.filter(published=True)
    published_competitions = reversed(
        sorted(
            published_competitions,
            key=lambda c: c.get_start_date()))
...
def sort_data_table(request, context, list):
    context['order'] = order = request.GET.get(
        'order') if 'order' in request.GET else 'id'
    context['direction'] = direction = request.GET.get(
        'direction') if 'direction' in request.GET else 'asc'
...
class CompetitionEdit(
        LoginRequiredMixin, NamedFormsetsMixin, UpdateWithInlinesView):
    ...
    def forms_valid(self, form, inlines):
        ...
        for phase_form in inlines[1]:
            if phase_form.cleaned_data["input_data_organizer_dataset"]:
                phase_form.instance.input_data = phase_form.cleaned_data[
                    "input_data_organizer_dataset"].data_file.file.name

            if phase_form.cleaned_data["reference_data_organizer_dataset"]:
                phase_form.instance.reference_data = phase_form.cleaned_data[
                    "reference_data_organizer_dataset"].data_file.file.name

            if phase_form.cleaned_data["scoring_program_organizer_dataset"]:
                phase_form.instance.scoring_program = phase_form.cleaned_data[
                    "scoring_program_organizer_dataset"].data_file.file.name

            phase_form.instance.save()

        # Look for admins that are not participants yet
        approved_status = models.ParticipantStatus.objects.get(
            codename=models.ParticipantStatus.APPROVED)
        ...
        for admin in form.instance.admins.all():
            try:
                ...
                models.CompetitionParticipant.objects.create(
                    user=admin,
                    competition=form.instance,
    ...
    def construct_inlines(self):
        ...
        for inline_form in inline_formsets[1].forms:
            ...
            input_data_ids = models.CompetitionPhase.objects.filter(
                competition=self.object).values_list('input_data_organizer_dataset')
            reference_data_ids = models.CompetitionPhase.objects.filter(
                competition=self.object).values_list('reference_data_organizer_dataset')
            scoring_program_ids = models.CompetitionPhase.objects.filter(
                competition=self.object).values_list('scoring_program_organizer_dataset')

            inline_form.fields['input_data_organizer_dataset'].queryset = models.OrganizerDataSet.objects.filter(
                Q(uploaded_by=self.request.user, type="Input Data") | Q(
                    pk__in=input_data_ids)
            )
            inline_form.fields['reference_data_organizer_dataset'].queryset = models.OrganizerDataSet.objects.filter(
                Q(uploaded_by=self.request.user, type="Reference Data") | Q(
                    pk__in=reference_data_ids)
            )
            inline_form.fields['scoring_program_organizer_dataset'].queryset = models.OrganizerDataSet.objects.filter(
                Q(uploaded_by=self.request.user, type="Scoring Program") | Q(
                    pk__in=scoring_program_ids)
            )
...
class CompetitionDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['public_submissions'] = models.CompetitionSubmission.objects.filter(
        ...
        try:
            ...
            if self.request.user.is_authenticated() and self.request.user in [
                    x.user for x in competition.participants.all()]:
                context['my_status'] = [x.status for x in competition.participants.all(
                ) if x.user == self.request.user][0].codename
                context['my_participant'] = competition.participants.get(
                    user=self.request.user)
                phase_iterator = iter(competition.phases.all())
                for phase in phase_iterator:
                    submissions[phase] = models.CompetitionSubmission.objects.filter(
                        participant=context['my_participant'],
                        phase=phase)
                    if phase.is_active:
                        context['my_active_phase_submissions'] = submissions[
                            phase]
...
class CompetitionSubmissionsPage(LoginRequiredMixin, TemplateView):
    ...
    # Requires an authenticated user who is an approved participant of the
    # competition.
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user in [
            ...
            if participant.status.codename == models.ParticipantStatus.APPROVED:
                ...
                submissions = models.CompetitionSubmission.objects.filter(
                    ...
                    phase=phase)
                # find which submission is in the leaderboard, if any and only
                # if phase allows seeing results.
                id_of_submission_in_leaderboard = -1
                if not phase.is_blind:
                    ids = [
                        e.result.id for e in models.PhaseLeaderBoardEntry.objects.filter(
                            board__phase=phase) if e.result in submissions]
                    if len(ids) > 0:
                        id_of_submission_in_leaderboard = ids[0]
        ...
        try:
            last_submission = models.CompetitionSubmission.objects.filter(
                participant=participant,
                phase=phase).latest('submitted_at')
            context['last_submission_team_name'] = last_submission.team_name
            context[
                'last_submission_method_name'] = last_submission.method_name
            context[
                'last_submission_method_description'] = last_submission.method_description
            context[
                'last_submission_project_url'] = last_submission.project_url
            context[
                'last_submission_publication_url'] = last_submission.publication_url
            context['last_submission_bibtex'] = last_submission.bibtex
            context[
                'last_submission_organization_or_affiliation'] = last_submission.organization_or_affiliation
        except ObjectDoesNotExist:
            pass
...
class CompetitionResultsDownload(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response = HttpResponse(
            competition.get_results_csv(
                phase.pk),
            status=200,
            content_type="text/csv")
        response[
            "Content-Disposition"] = "attachment; filename=%s results.csv" % phase.competition.title
        return response
    ...
    def get(self, request, *args, **kwargs):
        ...
        for group in groups:
            ...
            if len(group['scores']) <= 0:
                ...
                leader_board_entries = models.PhaseLeaderBoardEntry.objects.filter(
                    board=leader_board).values_list(
                    'result__id',
                    flat=True)

                for pk, scores in group['scores']:
                    submission = models.CompetitionSubmission.objects.get(
                        pk=scores['id'])
                    row = [scores['username']]
...
class MyCompetitionParticipantView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        participant_list = []
        competition_participants = self.queryset.filter(
            competition=self.kwargs.get('competition_id'))
        competition_participants_ids = list(
            participant.id for participant in competition_participants)
        context['pending_participants'] = filter(
            lambda participant_submission: participant_submission.status.codename == models.ParticipantStatus.PENDING,
            competition_participants)
        participant_submissions = models.CompetitionSubmission.objects.filter(
            participant__in=competition_participants_ids)
        for number, participant in enumerate(competition_participants):
            participant_entry = {
                ...
                'number': number + 1,
                # equivalent to assigning participant.submissions.count() but
                # without several multiple db queires
                'entries': len(filter(lambda participant_submission:
                                      participant_submission.participant.id == participant.id, participant_submissions))
            }
...
class MySubmissionResultsPartial(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['my_active_phase_submissions'] = phase.submissions.filter(
            participant=participant)
...
class MyCompetitionSubmissionOutput(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return HttpResponse(
                msg % filetype, status=200, content_type='text/plain')
...
class MyCompetitionSubmissionDetailedResults(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context_dict = {
            ...
            'filename': submission.detailed_results_file.name}
        return render_to_response(
            'web/my/detailed_results.html',
            context_dict,
...
class MyCompetitionSubmissionsPage(LoginRequiredMixin, TemplateView):
    ...
    # Requires an authenticated user who is an administrator of the
    # competition.
    queryset = models.Competition.objects.all()
    model = models.Competition
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.id == competition.creator_id or self.request.user in competition.admins.all(
            ...
            submissions = models.CompetitionSubmission.objects.filter(
                phase=active_phase)
            # find which submissions are in the leaderboard, if any and only if
            # phase allows seeing results.
            id_of_submissions_in_leaderboard = [
                e.result.id for e in models.PhaseLeaderBoardEntry.objects.all() if e.result in submissions]
            # create column definition
            columns = [
                {
                    'label': 'SUBMITTED',
            ...
            for submission in submissions:
                ...
                if (submission_info['is_in_leaderboard']):
                    for score_group_index, score_group in enumerate(scores):
                        user_score = filter(
                            lambda user_score: user_score[1][
                                'username'] == submission.participant.user.username,
                            score_group['scores'])[0]
                        main_score = filter(
                            lambda main_score: main_score[
                                'name'] == score_group['selection_key'],
                            user_score[1]['values'])[0]
                        submission_info[
                            'score_' +
                            str(score_group_index)] = main_score['val']
                submission_info_list.append(submission_info)
            # order results
    ...
    def get_context_data(self, **kwargs):
        ...
        for bundle in bundles:
            ...
            if 'metadata' in bundle:
                ...
                for (
                        key1, key2) in [
                        ('title', 'name'), ('creator', None), ('description', None)]:
                    if key2 is None:
                        key2 = key1
                    if key2 in metadata:
...
def BundleDownload(request, uuid):
    ...
    response = StreamingHttpResponse(
        ...
        content_type="zip")
    response[
        'Content-Disposition'] = 'attachment; filename="%s.zip"' % item['metadata']['name']
    return response
...
class OrganizerDataSetListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return models.OrganizerDataSet.objects.filter(
...
class OrganizerDataSetCheckOwnershipMixin(LoginRequiredMixin):
    ...
    def get_object(self, queryset=None):
        dataset = super(
            OrganizerDataSetCheckOwnershipMixin,
...
class OrganizerDataSetDelete(OrganizerDataSetCheckOwnershipMixin, DeleteView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.object.type == "Input Data":
            usage = usage.filter(
                phases__input_data_organizer_dataset=self.object)
        elif self.object.type == "Reference Data":
            usage = usage.filter(
                phases__reference_data_organizer_dataset=self.object)
        elif self.object.type == "Scoring Program":
            usage = usage.filter(
                phases__scoring_program_organizer_dataset=self.object)
        else:
            usage = usage.filter(
                Q(
                    phases__input_data_organizer_dataset=self.object) | Q(
                    phases__reference_data_organizer_dataset=self.object) | Q(
                    phases__scoring_program_organizer_dataset=self.object))
        ...
        if dataset.sub_data_files.count() > 0:
            ...
            for sub_dataset in dataset.sub_data_files.all():
                file_dir, file_name = os.path.split(
                    sub_dataset.data_file.file.name)
            ...
            resp[
                'Content-Disposition'] = 'attachment; filename=%s.zip' % dataset.name
            return resp
        else:
            mime = MimeTypes()
            ...
            if file_type != 'text/plain':
                response[
                    'Content-Disposition'] = 'attachment; filename="{0}"'.format(dataset.data_file.file.name)
            return response
    except:
...
def datasets_delete_multiple(request):
    ...
    for dataset_id in ids_to_delete:
        try:
            dataset = models.OrganizerDataSet.objects.get(
                pk=int(dataset_id),
...
def download_competition_yaml(request, competition_pk):
    try:
        ...
        response = HttpResponse(
            competition.original_yaml_file,
            content_type="text/yaml")
        response[
            'Content-Disposition'] = 'attachment; filename="competition_%s.yaml"' % competition_pk
        return response
    except ObjectDoesNotExist:
        return HttpResponse(status=404)
...
def download_competition_bundle(request, competition_pk):
    ...
    try:
        ...
        for p in competition.pagecontent.pages.all():
            if p.codename in yaml_data["html"].keys(
            ) or p.codename == 'terms_and_conditions' or p.codename == 'get_data':
            ...
            for phase_index, phase_yaml in yaml_data["phases"].items():
                if phase_yaml["phasenumber"] == phase.phasenumber:
                    if phase.reference_data and phase.reference_data.file.name not in file_name_cache:
                        yaml_data["phases"][phase_index][
                            "reference_data"] = phase.reference_data.file.name
                        file_name_cache += phase.reference_data.file.name
                        zip_file.writestr(
                            phase.reference_data.file.name,
                            phase.reference_data.file.read())

                    if phase.input_data and phase.input_data.file.name not in file_name_cache:
                        yaml_data["phases"][phase_index][
                            "input_data"] = phase.input_data.file.name
                        file_name_cache += phase.input_data.file.name
                        zip_file.writestr(
                            phase.input_data.file.name,
                            phase.input_data.file.read())

                    if phase.scoring_program and phase.scoring_program.file.name not in file_name_cache:
                        yaml_data["phases"][phase_index][
                            "scoring_program"] = phase.scoring_program.file.name
                        file_name_cache += phase.scoring_program.file.name
                        zip_file.writestr(
                            phase.scoring_program.file.name,
                            phase.scoring_program.file.read())
        ...
        resp = HttpResponse(
            ...
            mimetype="application/x-zip-compressed")
        resp[
            'Content-Disposition'] = 'attachment; filename=%s-%s.zip' % (competition.title, competition.pk)
        return resp
    except:
        exc_type, exc_value, exc_traceback = sys.exc_info()
        print "*** print_tb:"
...
def download_leaderboard_results(request, competition_pk, phase_pk):
    try:
        ...
        leaderboard_entries = models.PhaseLeaderBoardEntry.objects.filter(
            board__phase=phase)
    except ObjectDoesNotExist:
        raise Http404()
    ...
    try:
        ...
        team_name_cache = {}
        team_name_string = ""
        for result in models.PhaseLeaderBoardEntry.objects.filter(
                result__participant__user__team_name__isnull=False,
        ...
        if team_name_string:
            zip_file.writestr(
                "team_names_and_members.txt",
                team_name_string.encode('utf8'))

        # Add each submission
        for entry in leaderboard_entries:
            submission = entry.result
            username_or_team_name = submission.participant.user.username if not \
                submission.participant.user.team_name else "Team %s " % submission.participant.user.team_name
            file_name = "%s - %s submission.zip" % (
                username_or_team_name, submission.submission_number)
            zip_file.writestr(file_name, submission.file.read())

            output_file_name = "%s - %s output.zip" % (
                username_or_team_name, submission.submission_number)
            zip_file.writestr(output_file_name, submission.output_file.read())

            profile_data_file_name = "%s - %s profile.txt" % (
                username_or_team_name, submission.submission_number)
            user_profile_data = {
            ...
            user_profile_data_string = '\n'.join(
                ['%s: %s' % (k, v) for k, v in user_profile_data.items()])
            zip_file.writestr(
                profile_data_file_name,
                user_profile_data_string.encode('utf-8'))
            ...
            submission_metadata_file_name = "%s - %s method.txt" % (
                username_or_team_name, submission.submission_number)
            submission_metadata_file_string = "\n".join(
                ["%s: %s" % (field, getattr(submission, field)) for field in metadata_fields])
            zip_file.writestr(
                submission_metadata_file_name,
                submission_metadata_file_string.encode('utf-8'))
        ...
        resp = HttpResponse(
            zip_buffer.getvalue(),
            mimetype="application/x-zip-compressed")
        resp['Content-Disposition'] = 'attachment; filename=%s-%s-results.zip' % (
            competition.title, competition.pk)
        return resp
    except:
        exc_type, exc_value, exc_traceback = sys.exc_info()
        print "*** print_tb:"
