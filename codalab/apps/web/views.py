...
# @login_required
# def download_starting_kit(request, pk):
#     try:
#         #phase = models.CompetitionPhase.objects.get(id=phase_pk)
#         #competition = phase.competition
#         competition = models.Competition.objects.get(pk=pk)
#
#         current_phase = None
#         next_phase = None
#         phases = competition.phases.all()
...
#
#         if request.user.id != competition.creator.id and request.user not in competition.admins.all():
#             #return render(request, {"starting_kit": phase.get_starting_kit})
#             return current_phase.get_starting_kit()
#
#     except models.CompetitionSubmission.DoesNotExist:
#         raise Http404()


def create_starting_kit(request, pk):
    try:
        #phase = models.CompetitionPhase.objects.get(id=phase_pk)
        #competition = phase.competition
        competition = models.Competition.objects.get(pk=pk)
        ...
        with open('test_file', 'w') as f:
            f.write('some text')
            current_phase.starting_kit.save('test', f)
