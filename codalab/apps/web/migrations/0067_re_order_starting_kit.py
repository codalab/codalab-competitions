...
class Migration(DataMigration):
    ...
    def backwards(self, orm):
        ...
        orm.Page.objects.filter(codename='get_starting_kit').exclude(rank=2).update(rank=2)
        orm.Page.objects.filter(codename='get_data').exclude(rank=0).update(rank=0)
        orm.Page.objects.filter(codename='submit_results').exclude(rank=1).update(rank=1)
