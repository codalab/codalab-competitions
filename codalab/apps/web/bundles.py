...
if len(settings.BUNDLE_SERVICE_URL) > 0:
    ...
    class BundleService():
        ...
        def get_bundle_info(self, uuid):
            # def get_bundle_infos(self, uuids, get_children=False,
            # get_host_worksheets=False, get_permissions=False):
            bundle_info = _call_with_retries(
                lambda: self.client.get_bundle_info(
                    uuid,
                    True,
                    True,
                    True))
            ...
            for spec in cls.METADATA_SPECS:
                ...
                if metadata[key] == '' or metadata[key] == []:
                    continue
                value = worksheet_util.apply_func(
                    spec.formatting,
                    metadata.get(key))
        ...
        def create_run_bundle(self, args, worksheet_uuid):
            ...
            targets = cli.parse_key_targets(
                self.client,
                worksheet_uuid,
                args.target_spec)

            new_bundle_uuid = self.client.derive_bundle(
                'run',
                targets,
                args.command,
                metadata,
        ...
        def upload_bundle_url(self, url, info, worksheet_uuid):
            ...
            new_bundle_uuid = self.client.upload_bundle_url(
                url,
                info,
                worksheet_uuid,
                True)
        ...
        def add_worksheet_item(self, worksheet_uuid, bundle_uuid):
            self.client.add_worksheet_item(
                worksheet_uuid,
                worksheet_util.bundle_item(bundle_uuid))
        ...
        def resolve_interpreted_items(self, interpreted_items):
            return _call_with_retries(
                lambda: self.client.resolve_interpreted_items(
                    ('test', 'test')))
        ...
        def delete_worksheet(self, worksheet_uuid):
            return _call_with_retries(
                lambda: self.client.delete_worksheet(worksheet_uuid))
        ...
        def _create_cli(self, worksheet_uuid):
            manager = CodaLabManager(
                temporary=True,
                clients={
                    settings.BUNDLE_SERVICE_URL: self.client})
        ...
        def http_status_from_exception(self, ex):
            # This is brittle. See
            # https://github.com/codalab/codalab/issues/345.
            if isinstance(ex, UsageError):
                return 404
            return 500
