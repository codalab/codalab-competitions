...
@pytest.fixture
def codalab_health_settings_object():
    health_settings = HealthSettings.objects.get_or_create(pk=1)[0]
    health_settings.emails = "test@test.com, test2@test.com"
    health_settings.save()
    return health_settings


@pytest.mark.django_db(transaction=False)
def test_health_status_triggers_when_jobs_in_queue_greater_than_25(mailoutbox, codalab_health_settings_object, client):
    with mock.patch('apps.health.views.get_health_metrics') as get_health_metrics_mock:
        get_health_metrics_mock.return_value = {
            "jobs_pending_count": 510,
            "jobs_lasting_longer_than_10_minutes": [_ for _ in range(0, 5)],
            "alert_emails": codalab_health_settings_object.emails,
        }

        client.get(reverse("health_status_check_thresholds"))

        assert len(mailoutbox) == 1
        assert "test@test.com" in mailoutbox[0].recipients()
        assert "test2@test.com" in mailoutbox[0].recipients()
        assert mailoutbox[0].subject == "Codalab Warning: Jobs pending > 25!"


@pytest.mark.django_db(transaction=False)
def test_health_status_triggers_when_a_job_takes_greater_than_10_minutes_to_process(mailoutbox, codalab_health_settings_object, client):
    with mock.patch('apps.health.views.get_health_metrics') as get_health_metrics_mock:
        get_health_metrics_mock.return_value = {
            "jobs_pending_count": 5,
            "jobs_lasting_longer_than_10_minutes": [_ for _ in range(0, 15)],
        ...
        assert "test@test.com" in mailoutbox[0].recipients()
        assert "test2@test.com" in mailoutbox[0].recipients()
        assert mailoutbox[0].subject == "Codalab Warning: Many jobs taking > 10 minutes!"


@pytest.mark.django_db(transaction=False)
def test_health_status_trigger_emails_without_commas_works(mailoutbox, codalab_health_settings_object, client):
    codalab_health_settings_object.emails = "test_only_one@test.com"
    codalab_health_settings_object.save()
    ...
    with mock.patch('apps.health.views.get_health_metrics') as get_health_metrics_mock:
        ...
        assert len(mailoutbox) == 1
        assert "test_only_one@test.com" in mailoutbox[0].recipients()
