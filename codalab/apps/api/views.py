...
class CompetitionAPIViewSet(viewsets.ModelViewSet):
    ...
    def participate(self, request, pk=None):
        ...
        request.DATA['agreed_terms']

        # If there is no registration required we just check to make sure they have agreed to the terms and conditions
        # which is done by the javascript before the ajax call, during form
        # validation.
        if comp.has_registration:
            status = webmodels.ParticipantStatus.objects.get(
                codename=webmodels.ParticipantStatus.PENDING)
        else:
            status = webmodels.ParticipantStatus.objects.get(
                codename=webmodels.ParticipantStatus.APPROVED)

        p, cr = webmodels.CompetitionParticipant.objects.get_or_create(
            user=self.request.user, competition=comp, defaults={'status': status, 'reason': None})
    ...
    def _get_userstatus(self, request, pk=None, participant_id=None):
        ...
        status = 200
        try:
            p = webmodels.CompetitionParticipant.objects.get(
                user=self.request.user, competition=comp)
...
competition_list = CompetitionAPIViewSet.as_view(
    {'get': 'list', 'post': 'create', 'post': 'participate', })
competition_retrieve = CompetitionAPIViewSet.as_view(
...
class CompetitionPhaseEditView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        comp = webmodels.Competition.objects.get(
            pk=kwargs.get('competition_id'))
        if serial.data['end_date']:
            comp.end_date = serial.data['end_date']
            comp.save()
        for p in serial.data['phases']:
            if p['phase_id']:
                webmodels.CompetitionPhase.objects.get(
                    pk=p['phase_id'], competition=comp)
            else:
                webmodels.CompetitionPhase.objects.create(
                    competition=comp, label=p['label'],
                    start_date=p[
                        'start_date'],
                    phasenumber=p[
                        'phasenumber'],
...
competitionphase_list = CompetitionPhaseAPIViewset.as_view(
    {'get': 'list', 'post': 'create'})
competitionphase_retrieve = CompetitionPhaseAPIViewset.as_view(
    {'get': 'retrieve', 'put': 'update', 'patch': 'partial_update'})
...
class CompetitionPageViewSet(viewsets.ModelViewSet):
    ...
    def dispatch(self, request, *args, **kwargs):
        if 'competition_id' in kwargs:
            self._pagecontainer_q = webmodels.PageContainer.objects.filter(
                object_id=kwargs['competition_id'],
    ...
    def new_pagecontainer(self, competition_id):
        try:
            ...
            raise Http404
        self._pagecontainer = webmodels.PageContainer.objects.create(
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not container:
            container = self.new_pagecontainer(
                self.kwargs.get('competition_id'))
        return super(CompetitionPageViewSet, self).create(request, *args, **kwargs)

competition_page_list = CompetitionPageViewSet.as_view(
    {'get': 'list', 'post': 'create'})
competition_page = CompetitionPageViewSet.as_view(
    {'get': 'retrieve', 'put': 'update', 'patch': 'partial_update'})
...
class CompetitionSubmissionViewSet(viewsets.ModelViewSet):
    ...
    def post_save(self, obj, created):
        if created:
            signals.do_submission.send(
                sender=webmodels.CompetitionSubmission, instance=obj)
    ...
    def removeFromLeaderboard(self, request, pk=None, competition_id=None):
        ...
        lbe = webmodels.PhaseLeaderBoardEntry.objects.get(
    ...
    def addToLeaderboard(self, request, pk=None, competition_id=None):
        ...
        if submission.participant.user != self.request.user:
            raise ParseError(detail='Invalid submission')
        response = dict()
        lb, _ = webmodels.PhaseLeaderBoard.objects.get_or_create(
        ...
        entries = webmodels.PhaseLeaderBoardEntry.objects.filter(
            board=lb, result__participant=participant)
        for entry in entries:
            entry.delete()
        lbe, cr = webmodels.PhaseLeaderBoardEntry.objects.get_or_create(
            board=lb, result=submission)
        response['status'] = (201 if cr else 200)
        return Response(response, status=response['status'], content_type="application/json")

competition_submission_retrieve = CompetitionSubmissionViewSet.as_view(
    {'get': 'retrieve'})
competition_submission_create = CompetitionSubmissionViewSet.as_view(
    {'post': 'create'})
...
leaderboard_list = LeaderBoardViewSet.as_view(
    {'get': 'list', 'post': 'create'})
leaderboard_retrieve = LeaderBoardViewSet.as_view(
    {'get': 'retrieve', 'put': 'update', 'patch': 'partial_update'})
