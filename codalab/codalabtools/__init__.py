...
class BaseWorker(object):
        ...
        while True:
            try:
                ...
                if msg is not None:
                    ...
                    task_args = data[
                        'task_args'] if 'task_args' in data else None
                    if task_type in self.vtable:
                        self.logger.info(
                            "Running task: id=%s task_type=%s",
                            task_id,
                            task_type)
                        self.vtable[task_type](task_id, task_args)
                        self.logger.info(
                            "Task complete: id=%s task_type=%s",
                            task_id,
                            task_type)
                    else:
                        self.logger.warning(
                            "Unknown task_type=%s for task with id=%s",
                            task_type,
                            task_id)
    ...
    def start(self):
        ...
        worker = multiprocessing.Process(
            target=self._message_receive_listen,
            args=(
                queue,
            ))
        worker.start()
        ...
        while True:
            ...
            # We don't want to shut off submissions in process, so only
            # terminate if we're waiting for a message
